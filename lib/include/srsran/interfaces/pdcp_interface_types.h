...
//-----------------------------------------2024.02.24-----------------------------
 typedef enum {present, absent, nulltype} sdap_header_t;

typedef struct{
int               pdu_session_id;
uint8_t           drb_id;
sdap_header_t     sdap_head_dl;
sdap_header_t     sdap_head_ul;
bool              is_default_drb;
uint8_t           add_qfi_num;
uint8_t           qfi_add;
uint8_t           rel_qfi_num;
...
typedef sdap_allocates sdap_allocate;
//----------------------------------------------------------------------------------



//-----------------------------2023.10.20---------------------------------------
enum class pdcp_dis_timer_t {
  ms900  = 900,
  ms1200 = 1200,
  ms1500 = 1500,
  ms3000 = 3000,
...
//enum class pdcp_integ_pro_e_ { enabled = 1 };
//enum class pdcp_ciph_dis_e_ { True = 1 };


class pdcp_confg_t
{
public:
   pdcp_confg_t(uint8_t           bearer_id_,
                pdcp_dis_timer_t  disc_timer_,
...

      {
        hdr_len_bytes = ceilf((float)sn_len / 8);
         ...
         pdcp_dis_timer_t discard_timer = pdcp_dis_timer_t::ms900;
         ...
         bool is_tm=false;
       bool operator==(const pdcp_confg_t& other)const 
       {
        return discard_timer == other.discard_timer and integ_pro == other.integ_pro and 
            ciph_dis == other.ciph_dis and
               status_report_required == other.status_report_required and other.bearer_id and other.sn_len;
       }
     
       bool operator!=(const pdcp_confg_t& other) const { return not(*this == other); }
       std::string get_rb_name() const
       {
        return (rb_type == PDCP_RB_IS_DRB ? "DRB" : "SRB") + std::to_string(bearer_id);
       }
};
//---------------------------------------------------------------------------------------------

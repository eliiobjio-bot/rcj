...
public:
  ...
  virtual void set_ul_type(int ul_type) = 0;
  virtual void voiceType_mode(int voiceType)=0;//trans for voiceType 2.4k or 4.8k
  
  virtual void multi_set_parameters(int i,int band_ID,int freq_ID,int slot_ID,int business_type=0,int multi_type=1)=0;

  virtual void set_parameters(int band_ID,int freq_ID,int slot_ID,int business_type=0)=0;//transfor parameters to phy
  virtual void set_uecategory(uint8_t ue_category_)=0;
  ...
  virtual void set_mch_period_stop(uint32_t stop) = 0;
     ...
     * @param pid 进程ID
     */
    virtual void trans_pid_to_phy(int pid) = 0;
...
public:
  ...
  struct phy_rrc_cfg_t {
    bool              configured = false; ///< Indicates whether PHY shall consider configuring this cell/carrier
       ////--------------------2023.10.28--------------------------
    srsran::phy_ch_cfog_t phy_ch_cofg = {};
    ////----------------------------------------------------
...
//--------------------2023.10.28-----------------------------
  virtual void s_set_config(uint16_t rnti, const phy_rrc_cfg_list_t& phy_cfg_list) = 0;
  //-----------------------------------------------------
  /**

...
public:
  ...
  virtual void add_bearer(uint16_t rnti, uint32_t lcid, const srsran::pdcp_config_t& cnfg)                 = 0;
     //-----------------------2023.10.20-------------------------------------------------------------------------------
  virtual void add_bearerer(uint16_t rnti, uint32_t lcid, const srsran::pdcp_confg_t& cnfg)                = 0;
  //----------------------------------------------------------------
  ...
  virtual bool sdap_cfgerer(uint16_t rnti,uint32_t lcid,const srsran::sdap_allocate& sdap_cfg)             = 0;
 // virtual void SDAP_TTCN_header(srsran::unique_byte_buffer_t* sdap_ttcn_header)                            = 0;
};

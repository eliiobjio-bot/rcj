...
public:
  ...
  virtual void mod_ttcn_lcid(uint32_t lcid, size_t lcid_size) = 0;
  virtual void reset_ttcn_lcid() = 0;
  virtual void pdcp_ttcnmsg_rlc_s(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg)=0;
  virtual void enable_rlc_testcase_related_configuration(uint16_t ttcn_rnti, uint32_t lcid,uint16_t rlc_testID,int pid)=0;
  virtual void TC_6115_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid)=0;
  virtual void TC_6116_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid)=0;
  virtual void TC_6117_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid)=0;
  virtual void psch_test(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t psch_pdu)=0;
};

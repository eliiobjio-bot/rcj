...
public:
  ...
  void write_pdu_s(uint8_t* payload, uint32_t nof_bytes)
  {
    if (suspended) {
      queue_rx_pdu(payload, nof_bytes);
        ...
        std::cout<<"---------------write--------11111111---------"<<std::endl;
  ...
  {
    if (suspended) {
      std::cout<<"@@@@@@@@@@@@---------rlc-common-write-sdu-s----1----@@@@@@@@@"<<std::endl;
      queue_tx_sdu(std::move(sdu));
    } else {
      std::cout<<"@@@@@@@@@@@@---------rlc-common-write-sdu-s----2----@@@@@@@@@"<<std::endl;
  ...
  virtual void mod_enble(bool ttcn_rlc_enble_)=0;
  //void mod_enble() {};
  virtual void test_ID_enble(uint16_t rlc_testID)=0;
  virtual void setRlcPid(int pid_)=0;
  virtual void TC_6115_RLC_Handle_t()=0;
  virtual void TC_6116_RLC_Handle_t()=0;
  virtual void TC_6117_RLC_Handle_t()=0;

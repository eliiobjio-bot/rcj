...
enum class ul_sch_lcid {
  ...
  SHORT_BSR = 0b01011,
  LONG_BSR       = 0b01100,
  HANDOVER_RES=0b01101,
  HANDOVER_CMP=0b01110,
  PHR_REPORT     = 0b01111,
...
public:
  ...
  uint32_t parse_packet(uint8_t* ptr)
  {
    ...
    do {
      ...
      //printf("max_subheaders%d\n",(int)max_subheaders);
      //printf("nof_subheaders time%d\n",nof_subheaders);
      //std::cout<<"nof_subheaders time"<<nof_subheaders<<std::endl;
...
public:
 ...
 //add sych
  ...
  uint16_t get_c_rnti();
   //**************wwh
  uint16_t get_s_rnti();
  uint16_t get_lrnti_bandid();
  uint16_t get_lrnti_freqid();
  uint16_t get_lrnti_srnti();
  //********************************

...
struct log_channel_config {
  ...
  log_channel_config(std::string n, char tag, bool should_print_context, bool status = true) :
    name(std::move(n)), tag(tag), should_print_context(should_print_context), enabled(status)
  {
  }

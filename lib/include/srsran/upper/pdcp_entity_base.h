...
public:
 ...
 //-------------2023.10.20---------------------
  virtual bool s_configure(const pdcp_confg_t& cnfg_) = 0;
  //-------------------------------------------------------------
...
/*
  bool wx_is_srb() { return cofg.rb_type == PDCP_RB_IS_SRB; }
  bool wx_is_drb() { return cofg.rb_type == PDCP_RB_IS_DRB; }
*/

  virtual bool sdap_cfg(const sdap_allocate& sdap_cfg) = 0;

  virtual void direct_write_rlc_sdu(uint32_t lcid, unique_byte_buffer_t ttcn_pdcp_msg) = 0;
  //virtual void pdcp_ttcnmsg_rlc(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg) =0;
...
uint32_t net_mode;
  ...
  //5.21 wcb
  virtual unique_byte_buffer_t get_icmp_data() =0;

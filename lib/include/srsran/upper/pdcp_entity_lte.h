...
public:
  ...
  bool s_configure(const pdcp_confg_t& cnfg_) ;
  void reset() override;
  void reestablish() override;

  //-------------2024.02.24--------------------------------------
  bool sdap_cfg(const sdap_allocate& sdap_cfg) override;
  bool is_cipher_encrypt=false;
  bool is_integrity_generate=false;
  bool is_cipher_decrypt=false;
  bool is_integrity_verify=false;

  std::map<int,int> qosToDRBMap;
  static int             sdap_dctype;  // 0:control pdu   1:data pdu
  static int             sdap_rqi; // 0:no action  1:inform nas layer
  static int             sdap_rdi; //0:no action   1:update qos to drb map


  static int               s_pdu_session_id;
  static sdap_header_t     s_sdap_head_dl;
  static sdap_header_t     s_sdap_head_ul;
  static bool              s_is_default_drb;
  static uint8_t           s_add_qfi_num;
  static uint8_t           s_qfi_add;
  static uint8_t           s_rel_qfi_num;
  static uint8_t           s_qfi_reale;
  static uint8_t           s_drb_id;
  //-------------------------------------------------------------
  ...
  //5.21 wcb
  srsran::unique_byte_buffer_t get_icmp_data();
  srsran::unique_byte_buffer_t icmp_data_info = srsran::make_byte_buffer();
  bool data_to_icmp_flag = false;
  ...
  void direct_write_rlc_sdu(uint32_t lcid, unique_byte_buffer_t ttcn_pdcp_msg);

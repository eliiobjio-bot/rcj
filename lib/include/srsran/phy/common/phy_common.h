...
#define SRSRAN_MAX_TDD_SF_CONFIGS (7u)
  ...
  typedef struct SRSRAN_API
  {
    ...
    bool equiFlag;   // 扩展标志
    bool udFlag;     // 上下行标志
    int payloadType; // 负载类型（控制数据, 0:语音）
    int PDCH_PSCH_type; //PDCH:2:PDCH1-1,3:PDCH1-2, 5:PSCH1-1,6:PSCH1-2,7:PSCH5-1,8:PSCH5-2
    uint8_t pduBits[400];
    int pduBitlen;
    uint16_t pui;
    int modeType;
    ...
    int DS_Type;
    int SF_i;             //扩频码索引
    int Subframe_offset;  //仅针对DS-PTDCH-T
    float Power;          //各信道功率
  } slot_sched_cfg_t;

  typedef struct SRSRAN_API
  {
    int Multi_cc_Max;  //载波个数指示（单载波按照slot配置，size可能超过1）
    int muilti_cc_Flag; //多载波标志
    int framID;   // 帧号
    int burstNum; // 帧号中的触发数(常规模式)    帧号中的信道个数（扩频模式）     
    slot_sched_cfg_t burstPrasCfg[16]; // 5个时隙的详细配置
    slot_sched_cfg_t mul_cc_burstPrasCfg[20][5]; // 16cc + 5个时隙的详细配置
    int Freq_size;                    // 传输频点个数
    int Freq_nof;                     // 传输频点个数
    int Slot_allocation;
    
    int Scrambling_num;              //基本扰玛（0.16.32.48）
    int k_s;                         //码组编号（0-11） 
    int ds_mode;
  } sched_t;
  ...
  typedef struct SRSRAN_API
  {
    ...
    int msg_len;
    int ul_type;
    int voice_type;//0:2.4k voice,1:4.8k voice(PDCH1-1)
    uint8_t ue_category;
    int pid;
    int band_ID;
    int freq_ID;
    ...
    int mode;
    int business_type;
    int frequency_cal_flag;
    float frequency;
    uint8_t msg[500];
    uint8_t mul_cc_msg[16][5][500];
	bool overtime_flag;
  } srsran_xw_ul_sf_cfg_t;
  ...
  typedef enum SRSRAN_API
  {
    SRSRAN_MIMO_DECODER_ZF,
    SRSRAN_MIMO_DECODER_MMSE
  } srsran_mimo_decoder_t;

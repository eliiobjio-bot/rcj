...
#include "cy_basesocket.h"
#include <string.h>
#include <thread>

template <uint16_t BUFFER_SIZE = AS_DEFAULT_BUFFER_SIZE>
class UDPSocket : public CYBaseSocket
{
public:
    ...
    explicit UDPSocket(bool useConnect = false, FDR_ON_ERROR, int socketId = -1): CYBaseSocket(onError, SocketType::UDP, socketId)
    {
        if (useConnect)
        {

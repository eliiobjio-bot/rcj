...
void write_pcap_eutra_thread_function(srsran::pcap_net*           pcap_handle,
...
void write_pcap_nr_thread_function(srsran::pcap_net*          pcap_handle,
...
int lte_mac_pcap_net_test()
{
  ...
  std::unique_ptr<srsran::pcap_net> pcap_handle = std::unique_ptr<srsran::pcap_net>(new srsran::pcap_net());
  TESTASSERT(pcap_handle->open("127.0.0.1", "127.0.0.1",8038, 8028) == SRSRAN_SUCCESS);
  TESTASSERT(pcap_handle->open("127.0.0.1", "127.0.0.1",8038, 8028) != SRSRAN_SUCCESS); // open again will fail

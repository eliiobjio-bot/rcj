...
namespace srsran
{
  ...
  uint8_t pdcp_entity_lte::s_qfi_add = 0;
  int pdcp_entity_lte::s_pdu_session_id = 0;
  bool pdcp_entity_lte::s_is_default_drb = 0;
  uint8_t pdcp_entity_lte::s_qfi_reale = 0;
  sdap_header_t pdcp_entity_lte::s_sdap_head_dl = present;
  sdap_header_t pdcp_entity_lte::s_sdap_head_ul = present;
  uint8_t pdcp_entity_lte::s_drb_id = 0;

  int pdcp_entity_lte::sdap_dctype = 0; // 0:control pdu   1:data pdu
  int pdcp_entity_lte::sdap_rqi = 0;    // 0:no action  1:inform nas layer
  int pdcp_entity_lte::sdap_rdi = 0;    // 0:no action   1:update qos to drb map
  ...
  pdcp_entity_lte::pdcp_entity_lte(srsue::rlc_interface_pdcp *rlc_,
                                   ...
                                   uint32_t lcid_) : pdcp_entity_base(task_sched_, logger), rlc(rlc_), rrc(rrc_), gw(gw_)
  {
  ...
  //-----------------------2023.10.20---------------------------------------------
  bool pdcp_entity_lte::s_configure(const pdcp_confg_t &cnfg_)
  {
    if (active)
    {
      // Already configured
    ...
    cofg = cnfg_;
    rb_name = cofg.get_rb_name();

    maximum_pdcp_sn = (1u << cofg.sn_len) - 1u;
    ...
    if (is_drb() && not rlc->rb_is_um(lcid) && cofg.discard_timer == pdcp_dis_timer_t::infinity)
    {
      logger.warning(
          "Setting discard timer to 1500ms, to avoid issues with lingering SDUs in the Unacknowledged SDUs map. LCID=%d",
          lcid);
      cofg.discard_timer = pdcp_dis_timer_t::ms900;
    }

    // Queue Helpers
    maximum_allocated_sns_window = (1u << cofg.sn_len) / 2u;

    logger.info("Init %s with bearer ID: %d", rb_name.c_str(), cofg.bearer_id);
    logger.info("SN len bits: %d, SN len bytes: %d, reordering window: %d, Maximum SN: %d, discard timer: %d ms",
                cofg.sn_len,
                cofg.hdr_len_bytes,
                reordering_window,
                maximum_pdcp_sn,
                static_cast<uint32_t>(cofg.discard_timer));
    logger.info("Status Report Required: %s", cofg.status_report_required ? "True" : "False");
  ...
  //-----------------------2024.02.24---------------------------------------------
  bool pdcp_entity_lte::sdap_cfg(const sdap_allocate &sdap_cfg)
  {
    s_qfi_add = sdap_cfg.qfi_add;
    s_is_default_drb = sdap_cfg.is_default_drb;
    s_qfi_reale = sdap_cfg.release_qfi;
    s_sdap_head_dl = sdap_cfg.sdap_head_dl;
    s_sdap_head_ul = sdap_cfg.sdap_head_ul;
    s_drb_id = sdap_cfg.drb_id;

    std::cout << "sdap_temp_cfg.qfi_add:" << static_cast<int>(s_qfi_add) << std::endl;
    std::cout << "sdap_temp_cfg.def_drb:" << s_is_default_drb << std::endl;
    std::cout << "sdap_temp_cfg.qfi_reale:" << static_cast<int>(s_qfi_reale) << std::endl;
    std::cout << "sdap_temp_cfg.sdap_head_dl:" << s_sdap_head_dl << std::endl;
    std::cout << "sdap_temp_cfg.sdap_head_ul:" << s_sdap_head_ul << std::endl;
    std::cout << "sdap_temp_cfg.drbid:" << static_cast<int>(s_drb_id) << std::endl;
  ...
  void pdcp_entity_lte::write_sdu(unique_byte_buffer_t sdu, int upper_sn)
  {
...
#if 0
    printf("No integerity protection as well as encrypted data\n");
    for (uint32_t i = 0; i < sdu->N_bytes; i++)
    {
      printf("Initial data 0x:%x\n", *(sdu->msg + i));
    }
#endif
    ...
    if (is_drb())
    {
      if (s_sdap_head_dl == present)
      {
        std::cout << "---sdap_tmep_cfg.qfi_add:" << static_cast<int>(s_qfi_add) << std::endl;
        uint8_t sdap_header = 0;
        sdap_header = ((sdap_rqi << 7) | (sdap_rdi << 6)) | s_qfi_add;
        std::cout << "---" << static_cast<int>(sdap_header) << std::endl;

        uint8_t *s_msg = new uint8_t[sdu->N_bytes + 1];
        s_msg[0] = sdap_header;
        for (uint32_t i = 0; i < sdu->N_bytes; i++)
        {
          s_msg[i + 1] = *(sdu->msg + i);
        }
        memcpy(sdu->msg, s_msg, sdu->N_bytes + 1);
        sdu->N_bytes += 1;
        delete[] s_msg;
      }
      else
      {
        std::cout << "this Data PDU without SDAP header" << std::endl;
      }
    }*/
    ...
    uint32_t used_sn;
    std::cout << "[PDCP]used_sn 1=" << used_sn << std::endl;
    std::cout << "[PDCP]upper_sn 1=" << upper_sn << std::endl;
    if (upper_sn == -1)
    {
        std::cout << "upper_sn == -1"  << std::endl;
      used_sn = st.next_pdcp_tx_sn; // Normal scenario
    }
    else
    {
        std::cout << "XXXXX upper_sn == -1"  << std::endl;
      used_sn = upper_sn; // SN provided by the upper layers, due to handover.
    }
    std::cout << "[PDCP]used_sn 2=" << used_sn << std::endl;
    ...
    {
      is_integrity_generate = true;
      integrity_generate(sdu->msg, sdu->N_bytes, tx_count, mac + 4);
    }

#if 0
    if(is_integrity_generate==true){
      printf("integerity protection data\n");
      for(uint32_t i=0;i<sdu->N_bytes;i++)
      {
        printf("integerity protection data 0x:%x\n",*(sdu->msg+i));
      }
    }
#endif
    ...
    //*******************************************************//
    /**
     *@brief  MAC-I(4G only srb have,so modify)
     *@author zhaohongjun
     *@date   2024/05/24
     */
    ...
    if (is_srb() || (is_drb() && !cofg.is_tm))
    {
      logger.info("[PDCP]append MAC-I");
      append_mac(sdu, mac);
    }
    //*********************************************************//
    if (encryption_direction == DIRECTION_TX || encryption_direction == DIRECTION_TXRX)
    {
      is_cipher_encrypt = true;
      cipher_encrypt(
...
#if 0
  if(is_cipher_encrypt==true){
      printf("encryptd info\n");
      for(uint32_t i=0;i<sdu->N_bytes;i++)
      {
        printf("PDCP enciphered data 0x:%x\n",*(sdu->msg+i));
      }
    }
#endif
  ...
  /// Li
  void pdcp_entity_lte::direct_write_rlc_sdu(uint32_t lcid, unique_byte_buffer_t ttcn_pdcp_msg)
  {
    rlc->write_sdu(lcid, std::move(ttcn_pdcp_msg));
  }

  // void pdcp_entity_lte::pdcp_ttcnmsg_rlc(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg) {

  //   std::cout<<"sdadfaaf"<<std::endl;
  //   rlc->pdcp_ttcnmsg_rlc(ttcn_rnti, lcid, std::move(ttcn_rlc_msg));
  // }
  ...
  // 5.21 wcb
  srsran::unique_byte_buffer_t pdcp_entity_lte::get_icmp_data()
  {
    unique_byte_buffer_t ans = srsran::make_byte_buffer();
    if (icmp_data_info->N_bytes != 0 && data_to_icmp_flag == true)
    {
      memcpy(ans->msg, icmp_data_info->msg, icmp_data_info->N_bytes);
      ans->N_bytes = icmp_data_info->N_bytes;
      data_to_icmp_flag = false;
    }
    else
    {
      ans->N_bytes = 0;
  ...
  void pdcp_entity_lte::handle_srb_pdu(srsran::unique_byte_buffer_t pdu)
  {
    ...
    if (is_cipher_decrypt == true)
    {
      printf("decrypt success");
      for (uint32_t i = 0; i < pdu->N_bytes; i++)
      {
        printf("decrypt data 0x:%x\n", *(pdu->msg + i));
      }
    }
    // Extract MAC
    uint8_t mac[8];
    ...
    // Pass to upper layer
    std::cout << "Pass to upper layer" << std::endl;
    std::cout << pdu->N_bytes << std::endl;
    for (int i = 0; i < (int)pdu->N_bytes; i++)
    {
      std::cout << (*(pdu->msg + i)) << std::endl;
    }
  ...
  {
    printf(" HANDLE UM DRB PDU \n");
    uint32_t sn = read_data_header(pdu);
    //***********************************************//
    /**
     *@brief  Add EXTACT MAC
     *@author zhaohongjun
     *@date   2024/05/24
     */
    ...
    // std::cout << "*********************************" << std::endl;
    //---------------------------------------2024.2.24-----------------------------
    // uint8_t sdap_ul_hdr =0;  //5.23 wcb temporary annotation
    // sdap_ul_hdr = *(pdu->msg);
    // uint8_t temp_qfi_add_num = sdap_ul_hdr & 0x3F;
    // sdap_dctype = sdap_ul_hdr & 0x80;

    // qosToDRBMap[temp_qfi_add_num] = s_drb_id;

    // for(uint32_t i=0;i<pdu->N_bytes-1;i++)
    // {
    //   *(pdu->msg+i) = *(pdu->msg+i+1);
    // }
    // pdu->N_bytes-=1;
    //------------------------------------------------------------------------------
    ...
    printf(" UM TO UP ICMP \n");
    // for (int i = 0; i < 30; i++)
    // {
    //   printf("UM_PDCP_TO_UP_ICMP_MSG[%d]: %x\n", i, pdu->msg[i]);
    // }
    memcpy(icmp_data_info->msg, pdu->msg, pdu->N_bytes);
    icmp_data_info->N_bytes = pdu->N_bytes;
    data_to_icmp_flag = true;
  }
  ...
  void pdcp_entity_lte::handle_am_drb_pdu(srsran::unique_byte_buffer_t pdu)
  {
    ...
    // 5.21 wcb
    // gw->write_pdu(lcid, std::move(pdu));
    printf(" AM TO UP ICMP \n");
    // for (int i = 0; i < (int)pdu->N_bytes; i++)
    // {
    //   printf("AM_PDCP_TO_UP_ICMP_MSG[%d]: %x\n", i, pdu->msg[i]);
    // }
    memcpy(icmp_data_info->msg, pdu->msg, pdu->N_bytes);
    icmp_data_info->N_bytes = pdu->N_bytes;
    data_to_icmp_flag = true;

...
void pdcp_entity_base::integrity_generate(uint8_t* msg, uint32_t msg_len, uint32_t count, uint8_t* mac)
{
  ...
  logger.debug(mac, 8, "MAC (generated)");
}
...
bool pdcp_entity_base::integrity_verify(uint8_t* msg, uint32_t msg_len, uint32_t count, uint8_t* mac)
{
  ...
  switch (sec_cfg.integ_algo) {
    case INTEGRITY_ALGORITHM_ID_EIA0:
      break;
    case INTEGRITY_ALGORITHM_ID_128_EIA1:
      security_128_eia1(&k_int[16], count, cfg.bearer_id - 1, cfg.rx_direction, msg, msg_len, mac_exp+4);
      break;
    case INTEGRITY_ALGORITHM_ID_128_EIA2:
      security_128_eia2(&k_int[16], count, cfg.bearer_id - 1, cfg.rx_direction, msg, msg_len, mac_exp+4);
      break;
    case INTEGRITY_ALGORITHM_ID_128_EIA3:
      security_128_eia3(&k_int[16], count, cfg.bearer_id - 1, cfg.rx_direction, msg, msg_len, mac_exp+4);
      break;
    default:
      break;
  }
  ...
  if (sec_cfg.integ_algo != INTEGRITY_ALGORITHM_ID_EIA0) {
    for (uint8_t i = 0; i < 8; i++) {
      if (mac[i] != mac_exp[i]) {
        ...
        logger.error(mac_exp, 8, "MAC mismatch (expected)");
        logger.error(mac, 8, "MAC mismatch (found)");
        is_valid = false;
        break;
      }
    }
...
void pdcp_entity_base::extract_mac(const unique_byte_buffer_t& pdu, uint8_t* mac)
{
  ...
  // Extract MAC
  memcpy(mac, &pdu->msg[pdu->N_bytes - 8], 8);
  pdu->N_bytes -= 8;
}
...
{
  // Check enough space for MAC
  if (sdu->N_bytes + 8 > sdu->get_tailroom()) {
    logger.error("Not enough space to add MAC-I");
    return;
  }

  // Append MAC
  memcpy(&sdu->msg[sdu->N_bytes], mac, 8);
  sdu->N_bytes += 8;
}
} // namespace srsran

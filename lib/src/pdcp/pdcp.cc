...
//-----------------2023.10.20------------------------------
int pdcp::add_bearer(uint32_t lcid, const pdcp_confg_t& cofg)
{

  if (valid_lcid(lcid)) {
    return pdcp_array[lcid]->s_configure(cofg) ? SRSRAN_SUCCESS : SRSRAN_ERROR;
  }
  ...
  if (not entity->s_configure(cofg)) {
  ...
  logger.info("Add %s%d (lcid=%d, sn_len=%dbits)",
              cofg.rb_type == PDCP_RB_IS_DRB ? "DRB" : "SRB",
              cofg.bearer_id,
              lcid,
              cofg.sn_len);
...
// int pdcp::sdap_mapping_modfiy(uint32_t lcid,uint8_t qfi)
// {
//     std::cout<<"sdap1 send SDU to ttcn  4444444"<<std::endl;
//   std::cout<<"QoS flow ID: " << static_cast<int>(sdapcfg.qfi_add)<<" for DRB ID : "<<static_cast<int>(qosToDRBMap[sdapcfg.qfi_add])<<std::endl;

//   if((qosToDRBMap[qfi]<1) && (sdapcfg.is_default_drb == true))
//   {
//     lcid =1;
//   }else{
//     lcid = qosToDRBMap[qfi];
//   }
//   return lcid;   
// }

int pdcp::sdap_cfger(uint32_t lcid,const sdap_allocate& sdap_cfg)
{
  sdapcfg.pdu_session_id = sdap_cfg.pdu_session_id;
  sdapcfg.drb_id         = sdap_cfg.drb_id;
  sdapcfg.is_default_drb = sdap_cfg.is_default_drb;
  sdapcfg.add_qfi_num    = sdap_cfg.add_qfi_num;
  sdapcfg.qfi_add        = sdap_cfg.qfi_add;
  sdapcfg.rel_qfi_num    = sdap_cfg.rel_qfi_num;
  sdapcfg.release_qfi    = sdap_cfg.release_qfi;

  std::cout<<"pdu_session_id = "<<sdapcfg.pdu_session_id<<std::endl;
  std::cout<<"drb_id = "<<static_cast<int>(sdapcfg.drb_id)<<std::endl;
  std::cout<<"is_default_drb = "<<static_cast<int>(sdapcfg.is_default_drb)<<std::endl;
  std::cout<<"add_qfi_num = "<<static_cast<int>(sdapcfg.add_qfi_num)<<std::endl;
  std::cout<<"qfi_add = "<<static_cast<int>(sdapcfg.qfi_add)<<std::endl;
  std::cout<<"rel_qfi_num = "<<static_cast<int>(sdapcfg.rel_qfi_num)<<std::endl;
  std::cout<<"release_qfi = "<<static_cast<int>(sdapcfg.release_qfi)<<std::endl;

  if(qosToDRBMap[sdapcfg.qfi_add] != sdapcfg.drb_id)
  {
    qosToDRBMap[sdapcfg.qfi_add] = sdapcfg.drb_id; //drb id可以重复相同，qos flow id不能重复，所以一个drb id 上可以有多个qos flow
    pdcp_entity_lte::sdap_rdi =1;
  }
  
  if(qosToDRBMap[sdapcfg.release_qfi] > 0)
  {
    qosToDRBMap[sdapcfg.release_qfi] = 0;
    pdcp_entity_lte::sdap_rdi =1;
    std::cout<< " QoS flow ID: " <<sdapcfg.release_qfi<<"cannot map on this drb"<<std::endl;
  }

  if (valid_lcid(lcid))
  {
    return pdcp_array[lcid]->sdap_cfg(sdap_cfg) ? SRSRAN_SUCCESS : SRSRAN_ERROR;
  }
...
void pdcp::add_bearer_mrb(uint32_t lcid, const pdcp_confg_t& cfg)
{
  if (not valid_mch_lcid(lcid)) {
    std::unique_ptr<pdcp_entity_lte> entity;
    entity.reset(new pdcp_entity_lte{rlc, rrc, gw, task_sched, logger, lcid});
    if (not entity->s_configure(cfg)) {
      logger.error("Can not configure PDCP entity");
      return;
    }
...
void pdcp::del_bearer(uint32_t lcid)
{
  ...
  if (valid_lcid(lcid)) {
    std::cout<<"pdcp del_bearer"<<std::endl;
    std::cout<<"pdcp del_bearer lcid: "<<lcid<<std::endl;
...
void pdcp::write_pdu(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  if (valid_lcid(lcid)) {
    pdcp_array.at(lcid)->write_pdu(std::move(pdu));
     ...
     std::cout<<"Dropping PDU, lcid= "<<lcid<<" doesnt exists"<<std::endl;

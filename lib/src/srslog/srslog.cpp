...
/// removed.
static bool        g_logger_enable = true;
...
void srslog::set_global_logger_status(bool s)
{
  g_logger_enable           = s;
  srslog_instance& instance = srslog_instance::get();

  auto vector_content = instance.get_channel_repo().contents();
  for (auto* pv : vector_content) {
    pv->set_enabled(g_logger_enable);
  }

  return;
}
bool srslog::get_global_logger_status()
{
  return g_logger_enable;
}
...
static basic_logger& fetch_basic_logger_helper(const std::string& id, sink& s, bool should_print_context)
{
  ...
  log_channel& error = fetch_log_channel_helper(
      ...
      log_channel_config{id, basic_logger_chan_tags[0], should_print_context, get_global_logger_status()});
  log_channel& warning = fetch_log_channel_helper(
      build_logger_name(id, basic_logger_chan_tags[1]),
      s,
      instance.get_backend(),
      log_channel_config{id, basic_logger_chan_tags[1], should_print_context, get_global_logger_status()});
  log_channel& info = fetch_log_channel_helper(
      build_logger_name(id, basic_logger_chan_tags[2]),
      s,
      instance.get_backend(),
      log_channel_config{id, basic_logger_chan_tags[2], should_print_context, get_global_logger_status()});
  log_channel& debug = fetch_log_channel_helper(
      build_logger_name(id, basic_logger_chan_tags[3]),
      s,
      instance.get_backend(),
      log_channel_config{id, basic_logger_chan_tags[3], should_print_context, get_global_logger_status()});
...
sink* srslog::create_udp_sink(const std::string& name, const std::string &remote_ip, uint16_t port)
{
  // srslog::console("fetch_stdout_sink: ");
  // std::cout <<"xxx--fetch_stdout_sink:" << name << std::endl;
  return srslog_instance::get()
      .get_sink_repo()
      .emplace(
          std::piecewise_construct,
          std::forward_as_tuple(name),
          std::forward_as_tuple(new cy_udp_sink(std::unique_ptr<log_formatter>(new text_formatter), remote_ip, port)))
      .get();

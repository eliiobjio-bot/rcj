...
ul_sch_lcid sch_subh::ul_sch_ce_type()
{
  ...
  printf("ul_sch_ce_type(),ret(lcid)==%d\n",(uint8_t)ret);
...
{
  nof_bytes = size;
  std::cout<<" nof_bytes = size = "<<nof_bytes<<std::endl;
}
...
uint32_t sch_subh::size_plus_header()
{
  if (is_sdu() || is_var_len_ce()) {
   ...
   //std::cout<<"size_plus_header nof_bytes = "<<nof_bytes<<std::endl;
...
//2024/8/16------------wwh
uint16_t sch_subh::get_lrnti_bandid()
{
  return (uint16_t)payload[0];
}
uint16_t sch_subh::get_lrnti_freqid()
{
  return (uint16_t)payload[1]&0xc0>>6;
}
uint16_t sch_subh::get_lrnti_srnti()
{
  return (uint16_t)payload[1]&0x3f;
}
//---------------------------------------------
...
bool sch_subh::read_subheader(uint8_t** ptr)
{
  ...
  // Skip R
  bool e_bit = (bool)(*(*ptr) & 0x20) ? true : false;
  //printf("0xsssssssssssssssssssssss%8x\n",(*(*ptr)));
  //printf("e_bit:%d\n",e_bit);
 //  std::cout<<"*(*ptr)"<<bitset<8>(*(*ptr))<<std::endl;
  ul_sch_lcid  t_lcid       = (ul_sch_lcid) (* (*ptr) & 0x1f);
  //(*(*ptr) & 0x1f==0x1f)
  lcid=(uint8_t)t_lcid;
  printf("mac lcid---- %d\n",lcid);
  if(lcid==8)
  {
    sychCI=(uint8_t) * (*ptr) & 0xc0;
    if(sychCI==64)
    {
      sychCI=1;
    }
    printf("subheader sych CI %d\n",sychCI);
  }
  if (is_sdu() || is_var_len_ce()||((*(*ptr) & 0x1f)==0x1f)) {
    ...
    // if (e_bit) {
      ...
      *ptr += 1;
      if (F_bit) {
        nof_bytes = (uint32_t) * (*ptr) & 0xff;
        std::cout<<"MAC header nof_bytes = "<<nof_bytes<<std::endl;
        *ptr += 1;
        printf("XX 0x%x\n",*(*ptr-1));
      }
    // } else {
    ...
    std::cout<<"mac 2222"<<std::endl;
    //std::cout<<"-------------@@@@@@@@@@###########-------------"<<std::endl;
    if(t_lcid==ul_sch_lcid::LONG_BSR){
    nof_bytes = 3;//sizeof_ce(lcid, parent->is_ul());
    }else if(t_lcid==ul_sch_lcid::LRNTI)
    {
      std::cout<<"mac 2222LRNTI"<<std::endl;
    nof_bytes = 2;
    }else{
     nof_bytes = 1;//sizeof_ce(lcid, parent->is_ul());
    }
     *ptr+=1;
  }
  return e_bit;
...
void sch_subh::read_payload(uint8_t** ptr)
{
  payload = *ptr;
  *ptr += nof_bytes;
  std::cout<<" zhj test read_payload = "<<nof_bytes<<std::endl;
  // for(int i=0;i<nof_bytes;i++)
  // {
  //   printf("read_payload 0x:%x\n",*(payload+i));
  // }
}

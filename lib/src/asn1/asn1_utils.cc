...
//         while (n_bits > 0)
//                 {
//                  uint8_t keepmask = 0xff << (8 - offset);

//                   if((8 - offset) > n_bits)
//                   {
//                          bit = (uint8_t )(val << (8 - offset - n_bits));
//                         //arr[i]	 = (arr[i] & keepmask) | bit;
//                         *ptr |=  bit;
//                         offset += n_bits;
//                         n_bits = 0;
...
//                   else
//                   {
//                         bit = (uint8_t)(val >> (n_bits - 8 + offset));
//                         //arr[i]	 = (arr[i] & keepmask) | bit;
//                         *ptr += bit;
//                         n_bits -= (8 - offset);
//                         offset = 0;
...
SRSASN_CODE bit_ref::pack_bytes(const uint8_t* buf, uint32_t n_bytes)
{
  ...
  if (ptr + n_bytes >= max_ptr) {
    log_error("Buffer size limit was achieved");
    std::cout<<"Buffer size limit was achieved"<<std::endl;
    return SRSASN_ERROR_ENCODE_FAIL;
  }
  if (offset == 0) {
    // Aligned case
...
SRSASN_CODE pack_unsupported_ext_flag(bit_ref& bref, bool ext)
{
  ...
  if (ext) {
    log_error("ASN extensions not currently supported");
     std::cout<<"ASN extensions not currently supported"<<std::endl;
    return SRSASN_ERROR_ENCODE_FAIL;
  }
  return SRSASN_SUCCESS;
}
...
SRSASN_CODE pack_enum(bit_ref& bref, uint32_t e, uint32_t nof_types, uint32_t nof_exts, bool has_ext)
{
  if (e >= nof_types) {
   ...
   std::cout<<  "The provided enum is not within the range of possible values "<<std::endl;
    return SRSASN_ERROR_ENCODE_FAIL;
  }
  SRSASN_CODE ret;
  if (has_ext) {
...
uint64_t to_number(const uint8_t* ptr, uint32_t nbits)
{
  ...
  uint32_t nof_bytes = ceil_frac(nbits, 8u);
  for (uint32_t i = 0; i < nof_bytes; ++i) {
    val += (uint64_t)ptr[i] << (i * 8u);//zhj change (uint64_t)
  }
  return val;
}

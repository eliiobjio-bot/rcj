...
namespace srsran
{
  ...
  //-------------------------2023.10.20--------------------
  srsran::rlc_config_t make_rlc_confg_t(const asn1::rrc::rlc_cofg_c &asn1_type)
  {
    srsran::rlc_config_t rlc_cofg;
    switch (asn1_type.type().value)
    {
    case asn1::rrc::rlc_cofg_c::types_opts::am:
      std::cout << "AAAAA AM" << std::endl;
      rlc_cofg.rlc_mode = rlc_mode_t::am;
      rlc_cofg.asam.t_poll_retran = asn1_type.am().ul_am_rlc.t_poll_retran.to_number();
      rlc_cofg.asam.poll_pdu = asn1_type.am().ul_am_rlc.poll_pdu.to_number();
      rlc_cofg.asam.poll_byte = asn1_type.am().ul_am_rlc.poll_byte.to_number() * 1000;
      rlc_cofg.asam.max_retx_thres_hold = asn1_type.am().ul_am_rlc.max_retx_thres_hold.to_number();
      rlc_cofg.asam.t_reord = asn1_type.am().dl_am_rlc.t_reord.to_number();
      std::cout << "rlc_cofg.asam.t_reord = " << (int)rlc_cofg.asam.t_reord << std::endl;
      rlc_cofg.asam.t_status_proh = asn1_type.am().dl_am_rlc.t_status_proh.to_number();
      break;
    case asn1::rrc::rlc_cofg_c::types_opts::um_bi_dir:
      std::cout << "AAAAA UM" << std::endl;
      rlc_cofg.rlc_mode = rlc_mode_t::um;
      rlc_cofg.aum.t_reord = asn1_type.um_bi_dir().dl_um_rlc.t_reord.to_number();
      std::cout << "rlc_cofg.aum.t_reord = " << (int)rlc_cofg.aum.t_reord << std::endl;
      rlc_cofg.um.rx_sn_field_length = rlc_umd_sn_size_t::size10bits;
      rlc_cofg.um.tx_sn_field_length = rlc_umd_sn_size_t::size10bits;
      rlc_cofg.um.rx_window_size = 512;
      rlc_cofg.um.rx_mod = 1024;
      rlc_cofg.um.tx_mod = 1024;
      break;

    case asn1::rrc::rlc_cofg_c::types_opts::tm:
      rlc_cofg.rlc_mode = rlc_mode_t::tm;
      // rlc_cofg.atm      = asn1_type.tm();
      break;
  ...
  ///
  srsran::pdcp_config_t make_srb_pdcp_config_t(const uint8_t bearer_id, bool is_ue, bool flag)
  {
    pdcp_config_t cfg(bearer_id,
                      PDCP_RB_IS_SRB,
                      is_ue ? SECURITY_DIRECTION_UPLINK : SECURITY_DIRECTION_DOWNLINK,
  ...
  //--------------------------------2023.10.20--------------------------------------------
  srsran::pdcp_confg_t make_drb_pdcp_confg_t(const uint8_t bearer_id, const pdcp_cofg_s &pdcp_cofg)
  {
    pdcp_dis_timer_t disc_timer = pdcp_dis_timer_t::infinity;
    if (pdcp_cofg.dis_timer_present)
    {
      switch (pdcp_cofg.dis_timer.to_number())
      {
      case 900:
        disc_timer = pdcp_dis_timer_t::ms900;
        break;
      case 1200:
        disc_timer = pdcp_dis_timer_t::ms1200;
        break;
      case 1500:
        disc_timer = pdcp_dis_timer_t::ms1500;
        break;
      case 3000:
        disc_timer = pdcp_dis_timer_t::ms3000;
        break;
      case 5100:
        disc_timer = pdcp_dis_timer_t::ms5100;
        break;
      case 2:
        disc_timer = pdcp_dis_timer_t::spare2;
        break;
      case 1:
        disc_timer = pdcp_dis_timer_t::spare1;
        break;
      default:
        disc_timer = pdcp_dis_timer_t::infinity;
        break;
      }
    }
    bool integ_pro = true;
    ...
    if (pdcp_cofg.rlc_am_present)
    {
      status_report_required = pdcp_cofg.rlc_am.stat_rep_req;
    }
    pdcp_confg_t cofg(bearer_id, disc_timer, PDCP_RB_IS_DRB, true, true, sn_len, false, false);
    return cofg;
  }
  //-----------------------------------------------------------------------------
...
namespace asn1
{
  namespace rrc
  {
     ...
     *************************/
    //--------------2023.10.20------------------------------
    uint8_t get_rrc_obj_id(const drb_to_add_modi_s &drb)
    {
      return drb.drb_id;
    }
    ...
    uint8_t set_rrc_obj_id(const drb_id_s &drb)
    {
      return drb.drb_id;
    }
    //-------------------------------------------------

...
SRSASN_CODE service_accept_t::pack(asn1::bit_ref& bref)
{
  ...
  if (pdu_session_reactivation_result_present_s == true) {
    HANDLE_CODE(bref.pack(ie_iei_pdu_session_reactivation_result, 8));
    HANDLE_CODE(pdu_session_reactivation_result.pack(bref));
  }
  if (pdu_session_reactivation_result_error_cause_present == true) {
  ...
  while (bref.distance_bytes_end() > 0) {
    ...
    switch (iei) {
      case ie_iei_pdu_session_status:
        ...
        break;
      case ie_iei_pdu_session_reactivation_result:
        pdu_session_reactivation_result_present_s = true;
        HANDLE_CODE(pdu_session_reactivation_result.unpack(bref));
        break;
      case ie_iei_pdu_session_reactivation_result_error_cause:
        pdu_session_reactivation_result_error_cause_present = true;
...
SRSASN_CODE close_ue_test_loop_t::pack(asn1::bit_ref& bref)
{
    HANDLE_CODE(ue_test_loop_mode.pack(bref));
    
    if(ue_test_loop_mode_a_lb_setup_present)
    {
        HANDLE_CODE(ue_test_loop_mode_a_lb_setup.pack(bref));
    }
    else if(ue_test_loop_mode_b_lb_setup_present)
    {
        HANDLE_CODE(ue_test_loop_mode_b_lb_setup.pack(bref));
    }

    return SRSASN_SUCCESS;
}

//activate_test_mode_t
SRSASN_CODE activate_test_mode_t::pack(asn1::bit_ref& bref)
{
    HANDLE_CODE(ue_test_loop_mode.pack(bref));
...
SRSASN_CODE nas_5gs_hdr::unpack_outer(asn1::cbit_ref& bref)
{
  ...
  switch (extended_protocol_discriminator) {
    case extended_protocol_discriminator_5gmm:
      HANDLE_CODE(bref.unpack(spare_1_2_version, 1));
      HANDLE_CODE(bref.advance_bits(3)); // spare
    ...
    case extended_protocol_discriminator_5gsm:
      ...
      HANDLE_CODE(message_type.unpack(bref));
      break;
    case extended_protocol_discriminator_test_loop:
      HANDLE_CODE(bref.advance_bits(4)); // 
      std::cout<< "unpack_outer extended_protocol_discriminator_test_loop"<<std::endl;
      unpack_enum<security_header_type_opts, 4>(bref, &security_header_type);
      if (security_header_type == plain_5gs_nas_message) {
        HANDLE_CODE(message_type.unpack(bref));
      } else {
...
SRSASN_CODE nas_5gs_hdr::unpack(asn1::cbit_ref& bref)
{
  ...
  if (security_header_type != plain_5gs_nas_message) {
    ...
    switch (inner_extended_protocol_discriminator) {
      case extended_protocol_discriminator_5gmm:
        HANDLE_CODE(bref.unpack(inner_spare_1_2_version, 1));
        HANDLE_CODE(bref.advance_bits(3)); // spare
      ...
      case extended_protocol_discriminator_5gsm:
        ...
        break;
      case extended_protocol_discriminator_test_loop:
        std::cout<< "unpack extended_protocol_discriminator_test_loop"<<std::endl;
        HANDLE_CODE(message_type.unpack(bref));
        break;
...
SRSASN_CODE nas_5gs_hdr::pack_outer(asn1::bit_ref& bref)
{
  ...
  switch (extended_protocol_discriminator) {
    case extended_protocol_discriminator_5gmm:
      ...
      HANDLE_CODE(bref.pack(spare_1_2_version, 1));
      HANDLE_CODE(bref.pack(0x0, 3)); // spare
      pack_enum<security_header_type_opts, 4>(bref, security_header_type);
      if (security_header_type == plain_5gs_nas_message) {
        HANDLE_CODE(message_type.pack(bref));
      } else {
        if(spare_1_2_version ==0x01) {
    ...
    case extended_protocol_discriminator_test_loop:
      ...
      {
        pack_enum<extended_protocol_discriminator_opts, 8>(bref, extended_protocol_discriminator_5gmm);
        HANDLE_CODE(bref.pack(spare_1_2_version, 1));
        HANDLE_CODE(bref.pack(0x0, 3)); // spare
        pack_enum<security_header_type_opts, 4>(bref, security_header_type);
        if (security_header_type == plain_5gs_nas_message) {
          printf("pack msg error! casue: extended_protocol_discriminator_test_loop is not plain_5gs_nas_message\n");
          return SRSASN_ERROR_DECODE_FAIL;  //此类消息必须进行加密
        } else {  
          if(spare_1_2_version ==0x01) {
            HANDLE_CODE(bref.pack(message_authentication_code, 32));
          }
...
SRSASN_CODE nas_5gs_hdr::pack(asn1::bit_ref& bref)
{
  pack_outer(bref);
  // TC mode head
  if(inner_extended_protocol_discriminator == extended_protocol_discriminator_test_loop && security_header_type != plain_5gs_nas_message) {
    pack_enum<extended_protocol_discriminator_opts, 8>(bref, inner_extended_protocol_discriminator);
    HANDLE_CODE(message_type.pack(bref));
  }
  else if (security_header_type != plain_5gs_nas_message) {   //MM or SM head
    pack_enum<extended_protocol_discriminator_opts, 8>(bref, inner_extended_protocol_discriminator);  //pack epd
    if (inner_extended_protocol_discriminator == extended_protocol_discriminator_5gsm) {
      ...
      HANDLE_CODE(bref.pack(inner_spare_1_2_version, 1));
      HANDLE_CODE(bref.pack(0x0, 3)); //MM head
    }
...
//TC
    case msg_types::options::close_ue_test_loop: {
    close_ue_test_loop_t* msg = srslog::detail::any_cast<close_ue_test_loop_t>(&msg_container);
    HANDLE_CODE(msg->pack(msg_bref));
    break;
    }
    case msg_types::options::active_test_mode: {
    activate_test_mode_t* msg = srslog::detail::any_cast<activate_test_mode_t>(&msg_container);
    HANDLE_CODE(msg->pack(msg_bref));
    break;
    }

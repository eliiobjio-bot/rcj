...
// Paging ::= SEQUENCE
SRSASN_CODE s_paging_s::pack(bit_ref& bref) const
{
  HANDLE_CODE(paging_ue_id.pack(bref));
  return SRSASN_SUCCESS;
}
SRSASN_CODE s_paging_s::unpack(cbit_ref& bref)
{
  HANDLE_CODE(paging_ue_id.unpack(bref));
  return SRSASN_SUCCESS;
}
void s_paging_s::to_json(json_writer& j) const
{
  j.start_obj();
  j.write_fieldname("pagingUE-Identity");
  paging_ue_id.to_json(j);
  j.end_obj();
}


// PCCH-MessageType ::=             Paging
SRSASN_CODE pcch_msg_type_s::pack(bit_ref& bref) const
{
  HANDLE_CODE(paging.pack(bref));
  return SRSASN_SUCCESS;
}
SRSASN_CODE pcch_msg_type_s::unpack(cbit_ref& bref)
{
  HANDLE_CODE(paging.unpack(bref));
  return SRSASN_SUCCESS;
}
void pcch_msg_type_s::to_json(json_writer& j) const
{
  j.start_array();
...
SRSASN_CODE s_pcch_msg_s::pack(bit_ref& bref) const
{
...
SRSASN_CODE s_pcch_msg_s::unpack(cbit_ref& bref)
{
...
void s_pcch_msg_s::to_json(json_writer& j) const
{
  j.start_array();
  j.start_obj();

...
uint8_t security_generate_k_gnb(const uint8_t* k_amf, const uint32_t nas_count, uint8_t* k_gnb)
{
  ...
  // NAS Count
  std::vector<uint8_t> nas_count1;
  nas_count1.resize(4);
  nas_count1[0] = (nas_count >> 24) & 0xFF;
  nas_count1[1] = (nas_count >> 16) & 0xFF;
  nas_count1[2] = (nas_count >> 8) & 0xFF;
  nas_count1[3] = nas_count & 0xFF;
  
  //3gpp access
    std::vector<uint8_t> access_type;
    access_type.resize(1);
    access_type[0]=0x01;

  if (kdf_common(FC_5G_KGNB_KN3IWF_DERIVATION, key, nas_count1, access_type, k_gnb) != SRSRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return SRSRAN_ERROR;
  }
  return SRSRAN_SUCCESS;

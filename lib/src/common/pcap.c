...
/* Packs MAC Contect to a buffer */
inline int XWNBIOT_PCAP_PACK_MAC_CONTEXT_TO_BUFFER(MAC_Context_Info_t* context, uint8_t* buffer, unsigned int length)
{
  int      offset = 0;
  uint16_t tmp16;
  ...
  /*****************************************************************/
  buffer[offset++] = (uint8_t)CY_LAYER_TYPE_MAC;
  buffer[offset++] = context->direction;

  /* RNTI */
  buffer[offset++] = MAC_XWNB_RNTI_TAG;
  tmp16            = htons(context->rnti);
  memcpy(buffer + offset, &tmp16, 2);
  offset += 2;

  /* UEId */
  buffer[offset++] = MAC_XWNB_UEID_TAG;
  tmp16            = htons(context->ueid);
  memcpy(buffer + offset, &tmp16, 2);
  offset += 2;

  buffer[offset++] = MAC_XWNB_PAYLOAD_TAG;
  return offset;
}
inline int XWNBIOT_PCAP_PACK_RRC_CONTEXT_TO_BUFFER(RRC_Context_Info_t* context, uint8_t* buffer, unsigned int length)
{
  int offset = 0;
  ...
  /*****************************************************************/
  buffer[offset++] = (uint8_t)context->nbiotMode;
  buffer[offset++] = (uint8_t)CY_LAYER_TYPE_RRC;
  buffer[offset++] = (uint8_t)context->direction;
  buffer[offset++] = (uint8_t)context->channel;
  uint16_t pkt_len = (uint16_t)length;
  pkt_len          = htons(pkt_len);
  memcpy(&buffer[offset], &pkt_len, sizeof(uint16_t));
  offset += 2;
  return offset;
}
inline int XWNBIOT_PCAP_PACK_NAS_CONTEXT_TO_BUFFER(NAS_Context_Info_t* context, uint8_t* buffer, unsigned int length)
{
  int offset = 0;
  ...
  /*****************************************************************/
  buffer[offset++] = (uint8_t)context->nbiotMode;
  buffer[offset++] = (uint8_t)CY_LAYER_TYPE_NAS;
  buffer[offset++] = (uint8_t)context->direction;
  buffer[offset++] = 0;
  uint16_t pkt_len = (uint16_t)length;
  pkt_len          = htons(pkt_len);

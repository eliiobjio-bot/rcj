...
namespace srsran
{
  unique_socket pcap_net::socket;
  pcap_net::pcap_net() : pcap_base() {}

  pcap_net::~pcap_net()
  {
    close();
  }
  uint32_t pcap_net::open(std::string client_ip_addr_,
                          ...
                          uint32_t ue_id_)
  {
    ...
    if (socket.is_open())
    {
      ...
      std::cout << "get bind addr:" << bind_addr_str.c_str() << " client addr: "<< client_ip_addr_.c_str() << std::endl;
  ...
  void pcap_net::set_remote(std::string client_ip_addr, uint16_t client_udp_port)
  {
    if (not net_utils::set_sockaddr(&client_addr, client_ip_addr.c_str(), client_udp_port))
    {
      logger.error("Invalid client_ip_addr: %s", client_ip_addr.c_str());
      return;
    }
  }
  uint32_t pcap_net::close()
  {
    {
      std::lock_guard<std::mutex> lock(mutex);
  ...
  void pcap_net::write_pdu(pcap_pdu_t &pdu)
  {
    if (pdu.pdu != nullptr && socket.is_open())
    {
      ...
      case srsran_rat_t::nr:
        ...
        break;
      case srsran_rat_t::weixing:
        write_mac_nbiot_pdu_to_net(pdu);
        break;
      default:
  ...
  void pcap_net::write_mac_lte_pdu_to_net(pcap_pdu_t &pdu)
  {
    int bytes_sent;
    uint32_t offset = 0;
  ...
  void pcap_net::write_mac_nbiot_pdu_to_net(pcap_pdu_t &pdu)
  {
    int bytes_sent;
    uint32_t offset = 0;
    uint8_t buffer[PCAP_CONTEXT_HEADER_MAX];

    if (pdu.module == CY_LAYER_TYPE_MAC)
    {
      offset += XWNBIOT_PCAP_PACK_MAC_CONTEXT_TO_BUFFER(&pdu.context, buffer, PCAP_CONTEXT_HEADER_MAX);
      //  not support temply
      return;
    }
    else if (pdu.module == CY_LAYER_TYPE_RRC)
    {
      offset += XWNBIOT_PCAP_PACK_RRC_CONTEXT_TO_BUFFER(&pdu.rrc_context, buffer, pdu.pdu->N_bytes);
    }
 	else if (pdu.module == CY_LAYER_TYPE_NAS) {
      offset += XWNBIOT_PCAP_PACK_NAS_CONTEXT_TO_BUFFER(&pdu.nas_context, buffer, pdu.pdu->N_bytes);
  	}
    else
    {
      return;
    }
    // logger.info("get module is %d", pdu.module);
    if (pdu.pdu.get()->get_headroom() < offset)
    {
      logger.error("PDU headroom is to small for adding context buffer");
      return;
  ...
  void pcap_net::write_mac_nr_pdu_to_net(pcap_pdu_t &pdu)
  {
    int bytes_sent;
    uint32_t offset = 0;
    uint8_t buffer[PCAP_CONTEXT_HEADER_MAX];

...
//#define LOCAL_QI
#ifdef LOCAL_QI
#define ONE_FRAME_NUM 691200
float realdata[ONE_FRAME_NUM];
float imagdata[ONE_FRAME_NUM];
#endif
int radio::init(const rf_args_t& args, phy_interface_radio* phy_)
{
  phy = phy_;
#ifdef LOCAL_QI
for (int i = 0; i< ONE_FRAME_NUM;i++) {
	realdata[i]=0;
	imagdata[i]=0;
}
	  ...
	  if (NULL == fpi) {
		  printf("cbs %s fp 111111error=======\n",__func__);
	  }
	  fpq = fopen("1152M.q","r");
	  if (NULL == fpq) {
		  printf("cbs %s fp 22222222error=======\n",__func__);
	  }
...
{
  logger.error("rx_now--------------------- 1  frame: %d ",rxd_time.get(0).full_secs);
...
logger.error("rx_now--------------------- 2  frame: %d ",rxd_time.get(0).full_secs);
...
logger.error("rx_now--------------------- 3  frame: %d ",rxd_time.get(0).full_secs);
  // Set new buffer size
...
logger.error("rx_now--------------------- 4  frame: %d ",rxd_time.get(0).full_secs);
...
logger.error("rx_now--------------------- 5  frame: %d ",rxd_time.get(0).full_secs);
  // Perform decimation
  if (ratio > 1) {
    for (uint32_t ch = 0; ch < nof_channels; ch++) {
...
logger.error("rx_now--------------------- end  frame: %d ",rxd_time.get(0).full_secs);
  return ret;
}
...
#ifdef LOCAL_QI
	cf_t* tx_data = buffer.get(0);

	for (int i = 0; i< ONE_FRAME_NUM; i++) {
		__real__ tx_data[i] = realdata[i];
		__imag__ tx_data[i] = imagdata[i];
	}

//	for (int i = ONE_FRAME_NUM-20; i< ONE_FRAME_NUM-10;i++) {
//		printf("cbs %s dataq[%d]: %f\n",__func__,i,__real__ tx_data[i]);
//	}
#endif

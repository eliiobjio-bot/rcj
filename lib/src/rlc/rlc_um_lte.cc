...
#define RX_MOD_BASE(x) (((x) - vr_uh - cfg.um.rx_window_size) % cfg.um.rx_mod) // zhj
...
namespace srsran
{
  ...
  rlc_um_lte::rlc_um_lte(srslog::basic_logger &logger,
                         ...
                         srsran::timer_handler *timers_) : rlc_um_base(logger, lcid_, pdcp_, rrc_, timers_)
  {
  }
  ...
  void rlc_um_lte::TC_6115_RLC_Handle_t()
  {
    std::cout << "RLC TC_6115 Enble!" << std::endl;
    TC_6115 = true;
  }
  void rlc_um_lte::TC_6116_RLC_Handle_t()
  {
    std::cout << "RLC TC_6116 Enble!" << std::endl;
    TC_6116 = true;
  }
  void rlc_um_lte::TC_6117_RLC_Handle_t()
  {
    std::cout << "RLC TC_6117 Enble!" << std::endl;
    TC_6117 = true;
  }

  void rlc_um_lte::test_ID_enble(uint16_t rlc_testID)
  {
    std::cout << "rlc_um_lte::test_ID_enble" << rlc_testID << std::endl;
    if (rlc_testID == 625)
    {
      ttcn_testID_625 = true;
    }
    else if (rlc_testID == 621)
    {
      ttcn_testID_621 = true;
    }
    else if (rlc_testID == 622)
    {
      printf("ttcn_testID_622");
      ttcn_testID_622 = true;
    }
    else if (rlc_testID == 623)
    {
      printf("ttcn_testID_623");
      ttcn_testID_623 = true;
    }
    else if (rlc_testID == 624)
    {
      printf("ttcn_testID_624");
      ttcn_testID_624 = true;
    }
    else if (rlc_testID == 628)
    {
      printf("ttcn_testID_628");
      ttcn_testID_628 = true;
    }
  }

  void rlc_um_lte::setRlcPid(int pid_)
  {
    rlcPid = pid_;
  }

  void rlc_um_lte::mod_enble(bool ttcn_rlc_enble_)
  {
  ...
  bool rlc_um_lte::configure(const rlc_config_t &cnfg_)
  {
    ...
    logger.info("%s configured in %s: t_reordering=%d ms",
                rb_name.c_str(),
  ...
  uint32_t rlc_um_lte::rlc_um_lte_tx::build_data_pdu(unique_byte_buffer_t pdu, uint8_t *payload, uint32_t nof_bytes)
  {
    ...
    while (pdu_space > head_len + 1 && tx_sdu_queue.size() > 0)
    {
      ...
      if (last_li > 127) {
        logger.info(" last li is larger than 127B, this rlc sdu or sdu segment maps the end of data field only");
        break;
      }
    ...
    if (parent->ttcn_testID_625 ||parent->TC_6115||parent->TC_6116|| parent->TC_6117)// 
    {
       //no add vt_us(RLC SN)
      std::cout << "vt_us no add in rlc_um" << std::endl;
    }
    else
    {
    ...
    if (parent->ttcn_testID_623 && header.sn == 2)
    {
      header.li[0] = 55;
    }
    if (parent->ttcn_testID_628 == true && header.sn == 0 && parent->rlcPid == 1)
    {
      header.fi = 1;
    }
    if (parent->ttcn_testID_628 == true && parent->rlcPid == 2)
    {
      header.sn = 1;
      header.fi = 2;
    }
    if (parent->ttcn_testID_624 && header.sn == 0)
    {
      printf("624header.sn = %d", header.sn);
      header.fi = 1;
      printf("624header.fi = %d", header.fi);
    }
    // Add header and TX
  ...
  bool rlc_um_lte::rlc_um_lte_rx::configure(const rlc_config_t &cnfg_, std::string rb_name_) // zhj
  {
    cfg = cnfg_;
    ...
    // configure timer
    if (cfg.aum.t_reord > 0)
    {
      reordering_timer.set(static_cast<uint32_t>(cfg.aum.t_reord) / 60, [this](uint32_t tid)
                           { timer_expired(tid); });
    }
  ...
  void rlc_um_lte::rlc_um_lte_rx::handle_data_pdu(uint8_t *payload, uint32_t nof_bytes)
  {
    ...
    if (reordering_timer.is_running())
    {
      if (RX_MOD_BASE(vr_ux) <= RX_MOD_BASE(vr_ur) || (!inside_reordering_window(vr_ux) && vr_ux != vr_uh))
      {
        ...
        auto duration =
            std::chrono::duration_cast<std::chrono::minutes>(s3_time - start_time);
        reordering_timer.stop();
      }
    }
    if (!reordering_timer.is_running())
      ...
      else
      {
        ...
        for (uint32_t i = 0; i < rx_window[vr_ur].header.N_li; i++)
        {
          ...
          else
          {
            ...
            {
              if (parent->ttcn_testID_625 == true)
              {

                // write TTCN header to sdu
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
                ...
                memcpy(temp + 11, &parent->ttcn_rlc_sn, 2);

                memcpy(rx_sdu->msg, temp, 24);
              }
              else if (parent->ttcn_testID_621 == true)
              {
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
                ...
                memcpy(temp + 11, &parent->ttcn_rlc_sn, 2);
                memcpy(temp + 21, &rx_window[vr_ur].header.fi, 1);
                memcpy(rx_sdu->msg, temp, 24);
              }
              else if (parent->ttcn_testID_622 == true)
              {
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
              ...
              else if (parent->ttcn_testID_628 == true)
              {
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
                ...
                memcpy(temp + 11, &parent->ttcn_rlc_sn, 2);
                std::cout << "333 rx_window[vr_ur].header.fi=" << rx_window[vr_ur].header.fi << std::endl;
                memcpy(temp + 21, &rx_window[vr_ur].header.fi, 1);
                memcpy(rx_sdu->msg, temp, 24);
              }
              else if (parent->ttcn_testID_623 == true)
              {
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
              ...
              else if (parent->ttcn_testID_624 == true)
              {
                rx_sdu->msg -= 24;
                rx_sdu->N_bytes += 24;
        ...
        else
        {
          ...
          else
          {
            ...
            if (parent->ttcn_testID_625 == true)
            {
              ...
              memcpy(temp + 11, &parent->ttcn_rlc_sn, 2);
              std::cout << "1111" << temp[11] << " " << temp[12] << std::endl;
            ...
            else if (parent->ttcn_testID_621 == true)
            {
              ...
              memcpy(temp + 11, &parent->ttcn_rlc_sn, 2);
              std::cout << "rx_window[vr_ur].header.fi=" << rx_window[vr_ur].header.fi << std::endl;
              memcpy(temp + 21, &rx_window[vr_ur].header.fi, 1);
              memcpy(rx_sdu->msg, temp, 24);
            }
            else if (parent->ttcn_testID_622 == true)
  ...
  void rlc_um_lte::rlc_um_lte_rx::iot_reassemble_rx_sdus()
  {

    if (!iot_rx_sdu)
    {
      iot_rx_sdu = make_byte_buffer();
      if (!iot_rx_sdu)
      {
        logger.error("Fatal Error: Couldn't allocate buffer in rlc_um::reassemble_rx_sdus().");
        return;
    ...
    if (iot_rx_window.end() != iot_rx_window.find(vr_ur))
    {

      // 假设 iot_rx_window[sn].buf->N_bytes 是要复制的字节数
      if (iot_rx_window[vr_ur].buf->N_bytes > 0)
      {
        // 确保 iot_rx_sdu->msg 的大小足够大
        if (iot_rx_sdu->get_tailroom() >= iot_rx_window[vr_ur].buf->N_bytes)
        {
          // 使用 memcpy 复制数据
          memcpy(iot_rx_sdu->msg + iot_rx_sdu->N_bytes, iot_rx_window[vr_ur].buf->msg, iot_rx_window[vr_ur].buf->N_bytes);
          iot_rx_sdu->N_bytes += iot_rx_window[vr_ur].buf->N_bytes;
        }
        else
        {
          logger.error("iot_rx_sdu buffer is too small to accommodate the data.");
        }
      }
      else
      {
        logger.warning("No data to copy from iot_rx_window.");
      }
    }
    else
    {
    ...
    if (iot_rx_window[vr_ur].header.si == rlc_nr_si_field_t::full_sdu || iot_rx_window[vr_ur].header.si == rlc_nr_si_field_t::last_segment)
    {
      if (cfg.iot_um.is_mrb)
      {
        pdcp->write_pdu_mch(lcid, std::move(iot_rx_sdu));
      }
      else
      {
        pdcp->write_pdu(lcid, std::move(iot_rx_sdu));
      }
      iot_rx_sdu = make_byte_buffer();
    }
    vr_ur = (vr_ur + 1) % cfg.iot_um.rx_mod;
  }
  ...
  //-----------------------IOT-----------------------
  bool rlc_um_lte::rlc_um_lte_rx::iot_inside_reordering_window(uint16_t sn)
  {
    if (cfg.iot_um.rx_window_size == 0 || iot_rx_window.empty())
    {
      return true;
    }
    if (RX_MOD_BASE(vr_uh - cfg.iot_um.rx_window_size) <= RX_MOD_BASE(sn) && RX_MOD_BASE(sn) < RX_MOD_BASE(vr_uh))
    {
      return true;
    }
    else
                                   ...
                                   rlc_umd_pdu_header_t *header)
  {
    ...
    uint8_t *ptr = payload;
    // Fixed part
    // std::cout<<"-----------rlc_um_read_data_pdu_header-------------value:"<<sn_size<<std::endl;
  ...
  //-----------------------------IOT------------------------------
  void iot_rlc_um_read_data_pdu_header(uint8_t *payload,
                                       uint32_t nof_bytes,
                                       iot_rlc_umd_pdu_header_t *header)
  {
  ...
  //-----------------------IOT------------------------
  uint32_t iot_rlc_um_packed_length()
  {
    uint32_t len = 0;
    len += 1;
    return len;

...
namespace srsran
{
  ...
  rlc_am_lte::rlc_am_lte(srslog::basic_logger &logger,
                         ...
                         srsran::timer_handler *timers_) : logger(logger), rrc(rrc_), pdcp(pdcp_), timers(timers_), lcid(lcid_), tx(this), rx(this)
  {
  }
  ...
  bool rlc_am_lte::configure(const rlc_config_t &cfg_)
  {
    ...
    //*****************************************************************************//
    /**
     *@brief  Modify RLC Configuration
     *@author zhaohongjun
     *@date   2024/05/24
     */

#if 0 // SRSRAN Source code
...
#if 1
    if (not rx.wx_configure(cfg.asam))
    {
      logger.error("Error configuring bearer (RX)");
      return false;
    }

    if (not tx.wx_configure(cfg))
    {
      logger.error("Error configuring bearer (TX)");
      return false;
    }
    ...
    logger.info("%s configured: t_poll_retx=%d, poll_pdu=%d, poll_byte=%d, max_retx_thresh=%d, "
                "t_reordering=%d, t_status_prohibit=%d",
                rb_name.c_str(),
                cfg.asam.t_poll_retran,
                cfg.asam.poll_pdu,
                cfg.asam.poll_byte,
                cfg.asam.max_retx_thres_hold,
                cfg.asam.t_reord,
                cfg.asam.t_status_proh);
    return true;
#endif
    //*****************************************************************************//
  }
  ...
  // ttcn
  void rlc_am_lte::test_ID_enble(uint16_t rlc_testID)
  {
    if (rlc_testID == 627)
    {
      ttcn_testID_627 = true;
    }
    else if (rlc_testID == 626)
    {
      ttcn_testID_626 = true;
    }
    else if (rlc_testID == 6213)
    {
      ttcn_testID_6213 = true;
    }
    else if (rlc_testID == 6211)
    {
      ttcn_testID_6211 = true;
    }
    else if (rlc_testID == 6212)
    {
      ttcn_testID_6212 = true;
    }
    else if (rlc_testID == 629)
    {
      ttcn_testID_629 = true;
    }
    else if (rlc_testID == 6210)
    {
      ttcn_testID_6210 = true;
    }
  }

  void rlc_am_lte::setRlcPid(int pid_)
  {
    rlcPid = pid_;
    std::cout << "rlcPid=" << rlcPid << std::endl;
  }
  ...
  {
    uint32_t read_bytes = tx.read_pdu(payload, nof_bytes); // 生成PDU
    // std::cout<<"--------nof-bytes--------:"<<nof_bytes<<std::endl;
    std::cout << "------------@@@@@@@@@@@@------------" << std::endl;
  ...
  {
    rx.write_pdu(payload, nof_bytes); // 处理PDU
  ...
  rlc_am_lte::rlc_am_lte_tx::rlc_am_lte_tx(rlc_am_lte *parent_) : parent(parent_),
  ...
  bool rlc_am_lte::rlc_am_lte_tx::configure(const rlc_config_t &cfg_)
  {
    ...
    std::cout << " cfg.t_status_prohibit = " << cfg.t_status_prohibit << std::endl;
    // configure timers
    if (cfg.t_status_prohibit > 0)
    {
      status_prohibit_timer.set(static_cast<uint32_t>(cfg.t_status_prohibit),
                                [this](uint32_t timerid)
                                { timer_expired(timerid); });
    }

    std::cout << " cfg.t_poll_retx = " << cfg.t_poll_retx << std::endl;
    if (cfg.t_poll_retx > 0)
    {
      poll_retx_timer.set(static_cast<uint32_t>(cfg.t_poll_retx), [this](uint32_t timerid)
                          { timer_expired(timerid); });
    }
  ...
  bool rlc_am_lte::rlc_am_lte_tx::wx_configure(const rlc_config_t &cfg_)
  {
    std::lock_guard<std::mutex> lock(mutex);
    if (cfg_.tx_queue_length > MAX_SDUS_PER_RLC_PDU)
    {
    ...
    std::cout << " wx_cfg.t_status_proh = " << wx_cfg.t_status_proh << std::endl;
    // configure timers
    if (wx_cfg.t_status_proh > 0)
    {
      status_prohibit_timer.set(static_cast<uint32_t>(wx_cfg.t_status_proh) / 60,
                                [this](uint32_t timerid)
                                { timer_expired(timerid); });
    }
    std::cout << " wx_cfg.t_poll_retran = " << wx_cfg.t_poll_retran << std::endl;
    if (wx_cfg.t_poll_retran > 0)
    {
      poll_retx_timer.set(static_cast<uint32_t>(wx_cfg.t_poll_retran) / 8, [this](uint32_t timerid)
                          { timer_expired(timerid); });
    }
  ...
  void rlc_am_lte::mod_enble(bool ttcn_rlc_enble_)
  {
    ttcn_rlc_enble = ttcn_rlc_enble_;
    std::cout << "ttcn_rlc_enble=ttcn_rlc_enble_; " << ttcn_rlc_enble << std::endl;
    ;
  }
  ...
  {
    logger.debug("rlc max retx_thresh is %d, now sn : [%d] rlc pdu retransmits [%d] times", wx_cfg.max_retx_thres_hold, sn, tx_window[sn].retx_count);
    if (tx_window[sn].retx_count == wx_cfg.max_retx_thres_hold)
    {
  ...
  int rlc_am_lte::rlc_am_lte_tx::write_sdu(unique_byte_buffer_t sdu)
  {
    ...
    std::cout << "-----NULL SDU pointer in write_sdu(----" << std::endl;
    if (sdu.get() == nullptr)
    {
      std::cout << "-----NULL SDU pointer in write_sdu(----" << std::endl;
    ...
    std::cout << " rlc: pdu->N_bytes =" << nof_bytes << std::endl;
  ...
  for(uint32_t  i = 0; i < nof_bytes; ++i) {
    ...
    std::cout << " zhj test 11.15 tx_sdu_queue =" << tx_sdu_queue.size() << std::endl;
    std::cout << " zhj test 11.15 tx_sdu_queue cap = " << tx_sdu_queue.size_bytes() << std::endl;
  ...
  uint32_t rlc_am_lte::rlc_am_lte_tx::read_pdu(uint8_t *payload, uint32_t nof_bytes)
  {
    ...
    if (do_status() && not status_prohibit_timer.is_running())
    {
      ...
      if (parent->ttcn_testID_6212 == true && parent->rx.TC6211_number < 5)
        return 0;
    ...
    std::cout<<"zhj 11.15 tx_window.size() = "<<tx_window.size()<<std::endl;
    if(retx_queue.empty()){
      std::cout<<"zhj 11.15 retx_queue.empty() !!!!"<<std::endl;
    }
    if (tx_window.size() >= RLC_AM_WINDOW_SIZE && retx_queue.empty())
    {
      std::cout << "tx_window.size() >= RLC_AM_WINDOW_SIZE && retx_queue.empty() !" << std::endl;
      retransmit_pdu(vt_a);
    }
    ...
    if (not retx_queue.empty())
    {
      std::cout << "not retx_queue.empty() !!!" << std::endl;
      int32_t pdu_size = build_retx_pdu(payload, nof_bytes);
      if (pdu_size > 0)
      {
        return pdu_size;
      }
      std::cout << "not retx_queue.empty() !!! 111" << std::endl;
    }
  ...
  {
    am_end_time = std::chrono::high_resolution_clock::now();
    auto duration3 =
        std::chrono::duration_cast<std::chrono::milliseconds>(am_end_time - am_s3_time);
    std::cout << "duration3 xx = " << duration3.count() << std::endl;
    ...
    if (poll_retx_timer.is_valid() && poll_retx_timer.id() == timeout_id)
    {
      ...
      auto duration1 =
          std::chrono::duration_cast<std::chrono::milliseconds>(am_end_time - am_s1_time);
      std::cout << "duration1 xx = " << duration1.count() << std::endl;

      auto duration2 =
          std::chrono::duration_cast<std::chrono::milliseconds>(am_end_time - am_s2_time);
      std::cout << "duration2 xx = " << duration2.count() << std::endl;
  ...
  bool rlc_am_lte::rlc_am_lte_tx::poll_required()
  {
...
#if 1
    std::cout << " wx_cfg.poll_pdu = " << wx_cfg.poll_pdu << std::endl;
    std::cout << " wx_cfg.poll_byte = " << wx_cfg.poll_byte << std::endl;
    if (wx_cfg.poll_pdu > 0 && pdu_without_poll > static_cast<uint32_t>(wx_cfg.poll_pdu))
    {
      return true;
    }
    if (wx_cfg.poll_byte > 0 && byte_without_poll > static_cast<uint32_t>(wx_cfg.poll_byte))
    {
      return true;
    }
     ...
     */
    if (wx_cfg.poll_pdu == 0 && wx_cfg.poll_byte == 0 && vt_s % poll_periodicity == 0)
    {
      return true;
    }
    return false;
  ...
  int rlc_am_lte::rlc_am_lte_tx::build_status_pdu(uint8_t *payload, uint32_t nof_bytes)
  {
...
#if 1 // modify
      std::cout << " wx_cfg.t_status_proh = " << wx_cfg.t_status_proh << std::endl;
      std::cout << " status_prohibit_timer.is_valid() = " << status_prohibit_timer.is_valid() << std::endl;
      if (wx_cfg.t_status_proh > 0 && status_prohibit_timer.is_valid())
      {
        // re-arm timer
        status_prohibit_timer.run();
      }
    ...
    std::cout << "rlc_am_lte::rlc_am_lte_tx::build_status_pdu:" << std::endl;
    for (int i = 0; i < pdu_len; i++)
    {
      printf("0x:%x\n", *(payload + i));
    }
    std::cout << "[RLC][Read]pdu_len=" << pdu_len << std::endl;
    return pdu_len;
  }
  ...
  int rlc_am_lte::rlc_am_lte_tx::build_retx_pdu(uint8_t *payload, uint32_t nof_bytes)
  {
    ...
    if (poll_required())
    {
      ...
      if (poll_retx_timer.is_valid())
      {
        ...
        std::cout << " poll_retx_timer run 1 " << std::endl;
        am_s1_time = std::chrono::high_resolution_clock::now();
        poll_retx_timer.run();
      }
    }
  ...
  int rlc_am_lte::rlc_am_lte_tx::build_segment(uint8_t *payload, uint32_t nof_bytes, rlc_amd_retx_t retx)
  {
    ...
    if (poll_required())
    {
      ...
      if (poll_retx_timer.is_valid())
      {
        std::cout << " poll_retx_timer run 2 " << std::endl;
        am_s2_time = std::chrono::high_resolution_clock::now();
        poll_retx_timer.run();
      }
    }
  ...
  int rlc_am_lte::rlc_am_lte_tx::build_data_pdu(uint8_t *payload, uint32_t nof_bytes) // 将pdu(包含头和data)写入payload
  {
    ...
    while (pdu_space > head_len && tx_sdu_queue.get_n_sdus() > 0 && header.N_li < MAX_SDUS_PER_PDU)
    {
      ...
      if (last_li > 127) {
        logger.info("The rlc sdu or sdu segment size is larger than 127B, only mapping the end of rlc pdu");
        break;
      }
      ...
      do
      {
        ...
        if (parent->ttcn_testID_6212 == true || parent->ttcn_testID_627 == true || parent->ttcn_testID_6211 == true || parent->ttcn_testID_6210 == true)
        {
        }
      ...
      if (parent->ttcn_testID_6211 && header.sn == 2)
      {
        memcpy(payload, tx_sdu->msg, tx_sdu->N_bytes);
        return tx_sdu->N_bytes;
      }
      if (parent->ttcn_testID_6212 && header.sn == 2)
      {
        memcpy(payload, tx_sdu->msg, tx_sdu->N_bytes);
        return tx_sdu->N_bytes;
      }
    ...
    if (poll_required())
    {
      ...
      if (poll_retx_timer.is_valid())
      {
        am_s3_time = std::chrono::high_resolution_clock::now();
        poll_retx_timer.run();
      }
    }

    if (parent->ttcn_testID_6213 == true)
    {
      printf("ttcn_testID_6213\n");
    }
    else
    {
      vt_s = (vt_s + 1) % MOD;
    ...
    if (parent->ttcn_testID_6211 && header.sn == 0)
    {
      header.p = 0;
    }
    if (parent->ttcn_testID_6211 && header.sn == 1)
    {
      header.p = 0;
    }
    if (parent->ttcn_testID_6212 && header.sn == 0)
    {
      header.p = 0;
    }
    if (parent->ttcn_testID_6212 && header.sn == 1)
    {
      header.p = 0;
    }
    uint8_t *ptr = payload;
    rlc_am_write_data_pdu_header(&header, &ptr);           // ptr向后移动header大小的字节数
    memcpy(ptr, buffer_ptr->msg, buffer_ptr->N_bytes);     // 将生成的PDU(包括头)存到payload位置
    int total_len = (ptr - payload) + buffer_ptr->N_bytes; //(ptr - payload): 头长度、N_bytes: 数据长度
  ...
  rlc_am_lte::rlc_am_lte_rx::rlc_am_lte_rx(rlc_am_lte *parent_) : parent(parent_),
                                                                  pool(byte_buffer_pool::get_instance()),
                                                                  logger(parent_->logger),
  ...
  bool rlc_am_lte::rlc_am_lte_rx::configure(rlc_am_config_t cfg_)
  {
    ...
    {
      reordering_timer.set(static_cast<uint32_t>(cfg.t_reordering), [this](uint32_t tid)
                           { timer_expired(tid); });
    }
  ...
  bool rlc_am_lte::rlc_am_lte_rx::wx_configure(s_rlc_am_cfg_t cfg_)
  {
    ...
    std::cout << "wx_cfg.t_reord = " << wx_cfg.t_reord << std::endl;
    if (wx_cfg.t_reord > 0)
    {
      reordering_timer.set(static_cast<uint32_t>(wx_cfg.t_reord) / 60, [this](uint32_t tid)
                           { timer_expired(tid); });
    }
  ...
  void rlc_am_lte::rlc_am_lte_rx::stop()
  {
    ...
    rx_window.clear();
    // std::cout << "rx_window.clear" << std::endl;
  }
  ...
  {
    std::cout << "rx_window Size=" << rx_window.size();
    std::cout << "vr_r=" << vr_r << std::endl;

    std::cout << "TC6211_number=" << TC6211_number << std::endl;
    if (parent->ttcn_testID_6211 == true && TC6211_number < 2)
    {
      std::cout << "LOSE" << std::endl;
      ++TC6211_number;
      return;
    }
    if (parent->ttcn_testID_6212 == true)
    {
      if (TC6211_number++ < 2)
        return;
    }
    ...
    if (!inside_rx_window(header.sn))
    {
      ...
      if ((!parent->ttcn_testID_6212) || TC6211_number > 4)
      {
        return;
      }
    }
  ...
  void rlc_am_lte::rlc_am_lte_rx::reassemble_rx_sdus()
  {
    ...
    while (rx_window.has_sn(vr_r))
    {
      ...
      for (uint32_t i = 0; i < rx_window[vr_r].header.N_li; i++)
      {
        ...
        if (rx_sdu->get_tailroom() >= len)
        {
          if ((rx_window[vr_r].buf->msg - rx_window[vr_r].buf->buffer) + len < SRSRAN_MAX_BUFFER_SIZE_BYTES)
          {
            ...
            // ttcn
            if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_627)
            {
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
              new_rx_sdu->N_bytes = 21;

              new_rx_sdu->msg[11] = rx_window[vr_r].header.sn & 0xFF;
              new_rx_sdu->msg[12] = (rx_window[vr_r].header.sn >> 8) & 0xFF;

              new_rx_sdu->msg[21] = rx_window[vr_r].header.fi; // 直接通过索引21（即第22位）赋值
              new_rx_sdu->N_bytes += 1;                        // 更新字节计数，加上新添加的字节

              // 现在将这两个字节添加到new_rx_sdu的缓冲区中
              new_rx_sdu->msg[22] = rx_window[vr_r].header.li[0] & 0xFF;
              new_rx_sdu->msg[23] = (rx_window[vr_r].header.li[0] >> 8) & 0xFF;
              new_rx_sdu->N_bytes += 2; // 更新N_bytes来反映添加了一个字节

              memcpy(new_rx_sdu->msg + 24, rx_sdu->msg, rx_sdu->N_bytes);
              new_rx_sdu->N_bytes += rx_sdu->N_bytes;
              parent->pdcp->write_pdu(parent->lcid, std::move(new_rx_sdu));
            }
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_626)
            {
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
            ...
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_6213)
            {
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
            ...
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_6211)
            {
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
            ...
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_6212)
            {
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
            ...
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_6210)
            {
              std::cout << "parent->ttcn_rlc_enble:" << parent->ttcn_rlc_enble << "parent->ttcn_testID_6210:" << parent->ttcn_testID_6210 << std::endl;
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
              new_rx_sdu->N_bytes = 21;
              ...
              new_rx_sdu->msg[21] = rx_window[vr_r].header.p; // 直接通过索引21（即第22位）赋值
              new_rx_sdu->N_bytes += 1;                       // 更新字节计数，加上新添加的字节
              ...
              memcpy(new_rx_sdu->msg + 24, rx_sdu->msg, rx_sdu->N_bytes);
              new_rx_sdu->N_bytes += rx_sdu->N_bytes;
              printf("11ttcn_testID_6213 rx_sdu:%d\n", rx_sdu->N_bytes);
              for (uint32_t i = 0; i < rx_sdu->N_bytes; i++)
              {
                printf("0x:%x\n", *(rx_sdu->msg + i));
              }
              parent->pdcp->write_pdu(parent->lcid, std::move(new_rx_sdu));
            }
            else if (parent->ttcn_rlc_enble == true && parent->ttcn_testID_629)
            {
              std::cout << "parent->ttcn_rlc_enble:" << parent->ttcn_rlc_enble << "parent->ttcn_testID_629:" << parent->ttcn_testID_629 << std::endl;
              unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
              // 将 17 个字节的数据复制到新缓冲区
              memcpy(new_rx_sdu->msg, parent->my_variable, 21);
              new_rx_sdu->N_bytes = 21;
              new_rx_sdu->msg[11] = (rx_window[vr_r].header.sn >> 8) & 0xFF;
              new_rx_sdu->msg[12] = rx_window[vr_r].header.sn & 0xFF;
              // new_rx_sdu->msg[11] = (rx_window[vr_r].header.sn >> 8) & 0xFF;
              printf("629 header.sn = %d\n", rx_window[vr_r].header.sn);
              printf("629 new_rx_sdu->msg[11] = %d\n", new_rx_sdu->msg[11]);
              printf("629 new_rx_sdu->msg[12] = %d\n", new_rx_sdu->msg[12]);
              ...
              memcpy(new_rx_sdu->msg + 24, rx_sdu->msg, rx_sdu->N_bytes);
              new_rx_sdu->N_bytes += rx_sdu->N_bytes;
              printf("11ttcn_testID_629 rx_sdu:%d\n", rx_sdu->N_bytes);
              for (uint32_t i = 0; i < rx_sdu->N_bytes; i++)
              {
                printf("0x:%x\n", *(rx_sdu->msg + i));
              }
      ...
      if (rlc_am_end_aligned(rx_window[vr_r].header.fi))
      {
        ...
        {
          std::cout << "parent->ttcn_rlc_enble:" << parent->ttcn_rlc_enble << "parent->ttcn_testID_6213:" << parent->ttcn_testID_6213 << std::endl;
          unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
          // 将 17 个字节的数据复制到新缓冲区
          memcpy(new_rx_sdu->msg, parent->my_variable, 21);
          new_rx_sdu->N_bytes = 21;
        ...
        {
          std::cout << "parent->ttcn_rlc_enble:" << parent->ttcn_rlc_enble << "parent->ttcn_testID_6211:" << parent->ttcn_testID_6211 << std::endl;
          unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
          // 将 17 个字节的数据复制到新缓冲区
          memcpy(new_rx_sdu->msg, parent->my_variable, 21);
          new_rx_sdu->N_bytes = 21;
        ...
        {
          std::cout << "parent->ttcn_rlc_enble:" << parent->ttcn_rlc_enble << "parent->ttcn_testID_6212:" << parent->ttcn_testID_6212 << std::endl;
          unique_byte_buffer_t new_rx_sdu = make_byte_buffer();
          // 将 17 个字节的数据复制到新缓冲区
          memcpy(new_rx_sdu->msg, parent->my_variable, 21);
          new_rx_sdu->N_bytes = 21;
    ...
    exit:
      ...
      if (!parent->ttcn_testID_6212 || TC6211_number >= 5)
      {
        vr_r = (vr_r + 1) % MOD;
        vr_mr = (vr_mr + 1) % MOD;
      }
  ...
  void rlc_am_lte::rlc_am_lte_rx::write_pdu(uint8_t *payload, const uint32_t nof_bytes)
  {
    ...
    else
    {
      ...
      rlc_am_read_data_pdu_header(&payload, &payload_len, &header); // 从payload读头，存入header, len减掉头的长度
  ...
  void rlc_am_lte::rlc_am_lte_rx::timer_expired(uint32_t timeout_id)
  {
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(s2_time - start_time);
  ...
  void rlc_am_read_data_pdu_header(uint8_t **payload, uint32_t *nof_bytes, rlc_amd_pdu_header_t *header)
  {
    ...
    if (RLC_DC_FIELD_DATA_PDU == header->dc)
    {
      ...
      while (ext)
      {
        ...
        ext = ((*ptr >> 7) & 0x01);
        header->li[header->N_li] = (*ptr & 0x7f);
        ptr++;
        header->N_li++;
      }
  ...
  uint32_t rlc_am_packed_length(rlc_amd_pdu_header_t *header)
  {
    ...
    if (header->rf)
    {
      len += 2; // Segment header is 2 bytes: (LSF + SO)
    }
    len += header->N_li; // Extension part - integer rounding up
    return len;
  }

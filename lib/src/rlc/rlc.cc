...
void rlc::write_sdu(uint32_t lcid, unique_byte_buffer_t sdu)
{
  ...
  if (valid_lcid(lcid)) {
    std::cout<<"down ---2----lcid:"<<lcid<<std::endl;
...
void rlc::write_pdu(uint32_t lcid, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (valid_lcid(lcid)) {
    ...
    update_bsr(lcid);
     ...
     //std::cout<<lcid<<"LCID %d doesn't exist. Dropping PDU"<<std::endl;
...
void rlc::del_bearer(uint32_t lcid)
{
  ...
  if (valid_lcid(lcid)) {
    ...
    printf("[RLC][del_bearer] Can't delete bearer with LCID %d. Bearer doesn't exist.\n", lcid);
  }
}

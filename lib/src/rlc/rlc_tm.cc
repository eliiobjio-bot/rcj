...
{
  std::cout<<"@@@@@@----------rlc_tm::write_sdu-----------@@@@@@"<<std::endl;
  if (!tx_enabled) {
    return;
  }
  if (sdu != nullptr) {
...
{
  std::cout<<"------------------read_pdu RLC TM ----------------------"<<std::endl;
  uint32_t pdu_size = ul_queue.size_tail_bytes();
  std::cout<<" Zhj test pdu_size = "<<pdu_size<<std::endl;
...
{
  std::cout<<"------------------ RLC TM ----------------------"<<std::endl;
  unique_byte_buffer_t buf = make_byte_buffer();
  if (buf != nullptr) {
    memcpy(buf->msg, payload, nof_bytes);
    buf->N_bytes = nof_bytes;
    for(uint8_t i=0; i<nof_bytes;i++)
    {
      printf("0x-----gxm------:%x\n",*(buf->msg + i));
    }
    buf->set_timestamp();
    ...
    if (srsran::srb_to_lcid(srsran::lte_srb::srb0) == lcid) {
      ...
      std::cout<<"ahfuhaoshfoahsfijhps"<<std::endl;

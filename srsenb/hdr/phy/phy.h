...
#include "srsenb/hdr/phy/mac_test.h"
#include "txrx.h"
...
public:
  ...
  std::string get_type() override { return "lte"; };
    ...
    void set_ul_type(int ul_type) final; // HM

    void trans_pid_to_phy(int pid) final;

    void multi_set_parameters(int i,int band_ID,int freq_ID,int slot_ID,int business_type=0,int multi_type=1)final;
  
    void voiceType_mode(int voiceType) final;
    void set_uecategory(uint8_t ue_category_) final;
    void set_parameters(int band_ID, int freq_ID, int slot_ID, int business_type = 0) final;
  ...
  //-------------------------2023.10.28------------------------------------------------
   void s_set_config(uint16_t rnti, const phy_rrc_cfg_list_t& phy_cfg_list) override;
  //------------------------------------------------------------------

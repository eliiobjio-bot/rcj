...
public:
  ...
  std::chrono::_V2::system_clock::time_point get_sched_start_time;
  std::chrono::_V2::system_clock::time_point get_sched_end_time;
  ...
  //V1.0.0###############################################
void my_test(stack_interface_phy_lte* stack, uint32_t tti);
   uint8_t  rach_test[14]={0x00,0x40,0x00,0x00,0x00,0x00,0x00,0x10,0x02,0x00,0x00,0x00,0x00,0x02};
//######################################################
 static int ul_type_;
 static int Voicetype_;
 static int band_ID_;
 static int freq_ID_;
 static int slot_ID_;
 static int multi_band_ID_[20];
 static int multi_slot_ID_[20];
 static int multi_freq_ID_[20];
 static int multi_cc_;
 
 static int business_type_;
 static uint8_t phy_ue_category_;
 static int pid_;

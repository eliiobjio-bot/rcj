...
namespace srsenb
{
  ...
  {
  public:
    struct TTCN_LCID_CONFG
    {
      bool ttcn_lcid_flag = false;
      uint32_t ttcn_lcid = 0;
      size_t ttcn_size = 0;
    };

    TTCN_LCID_CONFG ttcn_lcid_cfg;
    ...
    void pdcp_ttcnmsg_rlc_s(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg);
    void enable_rlc_testcase_related_configuration(uint16_t ttcn_rnti, uint32_t lcid, uint16_t rlc_testID,int pid);
    void TC_6115_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid);
    void TC_6116_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid);
    void TC_6117_RLC_Handle(uint16_t ttcn_rnti,uint32_t lcid);
    void psch_test(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t psch_pdu);

    // void pdcp_ttcnmsg_rlc(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg);
    void mod_ttcn_lcid(uint32_t lcid, size_t lcid_size);
    void reset_ttcn_lcid();

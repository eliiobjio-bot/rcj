...
typedef struct{
    ...
    int roid = 0b00101011;
    int rnti;
    bool is_handover_falg=false;
    uint8_t lrnti_bandid;
    uint8_t lrnti_freqid;
    uint8_t lrnti_srnti;
  }handover_para_t;
static handover_para_t* handover_para=(handover_para_t *)malloc(sizeof(handover_para_t));
//-----------------------------------------------------------------------------------------------
...
public:
  ...
  typedef struct 
  {
    ...
    uint8_t mcs=0;
    uint8_t fn_mcs=0;
    bool is_mcs_sent=false; //指示组包时 MCS的头
  }mcs_adjust_t;

  mcs_adjust_t mcs_adjust;
...
//V1.0.3#####################################################
  void parse_packet(srsran::unique_byte_buffer_t pdu);
   uint32_t get_ci_check_state();
 // ########################################################
  ...
  uint32_t read_pdu(uint32_t lcid, uint8_t* payload, uint32_t requested_bytes);
  uint32_t generate_mcs_ce(uint32_t &lcid, uint8_t* payload);
  handover_para_t* get_handover_para();
private:

...
//------------------------------------2023.10.27--------------------------
void mac_controller::s_handle_con_reconf_complete()
{
  //current_sched_ue_cfg = next_sched_ue_cfg;
  s_apply_current_bearers_cfg();
  update_mac();
  mac->phy_config_enabled(rnti, true);
...
//---------------------------2023.10.27---------------------------------
void mac_controller::s_apply_current_bearers_cfg()
{
  const drb_to_add_modi_list_s& drbs = bearer_list.s_get_established_drbs();
  for (const drb_to_add_modi_s& drb : drbs) {
    auto& bcfg = current_sched_ue_cfg.ue_bearers[drb.drb_id];
    bcfg       = {};
    bcfg.direction = mac_lc_ch_cfg_t::BOTH;
    bcfg.group     = 1;
    bcfg.priority  = 4;
    
    if (drb.log_chan_cfg_present and drb.log_chan_cfg.ul_spec_para_present) {
      bcfg.group = drb.log_chan_cfg.ul_spec_para.log_chan_goup;
      bcfg.pbr   = drb.log_chan_cfg.ul_spec_para.prio_bit_rate;
      bcfg.priority = drb.log_chan_cfg.ul_spec_para.priority;
      bcfg.bsd      = drb.log_chan_cfg.ul_spec_para.buck_size_dura;
    }
  }

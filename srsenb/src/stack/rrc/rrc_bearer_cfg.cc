...
//-------------------------------------------------2023/10/12-----------------------------------------------------------//
asn1::rrc::sec_alg_cfg_s security_cfg_handler::get_sec_alg_cfg()
{
  asn1::rrc::sec_alg_cfg_s ret;
  // TODO: select these based on UE capabilities and preference order
  ret.integ_prot_alg_present = true;
  ret.coph_alg       = (sec_alg_cfg_s::ciph_alg_e_::options)sec_cfg.cipher_algo;
  ret.intef_prot_alg = (sec_alg_cfg_s::intef_prot_alg_e_::options)sec_cfg.integ_algo;
  return ret;
}
//----------------------------------------------------------------------------------------------
...
bool security_cfg_handler::set_security_capabilities(const asn1::s1ap::ue_security_cap_s& caps,bool ttcn_test_enble,bool TC_79_is_paging_smc)
{
...
//zhj0528 zhanshi for benke
if(ttcn_test_enble==true)
{
  std::cout<<"s1"<<std::endl;
  sec_cfg.cipher_algo = srsran::CIPHERING_ALGORITHM_ID_EEA0;
  sec_cfg.integ_algo = srsran:: INTEGRITY_ALGORITHM_ID_EIA0;//zhj321
}
  return true;
}
...
//----------------------------------------2023.10.13----------------------------------------
void bearer_cfg_handler::s_fill_pending_nas_info(asn1::rrc::rrc_con_recfg_r1_ies_s* msg)
{
  // Add space for NAS messages
  uint8_t n_nas = erab_info_list.size();
  if (n_nas > 0) {
    msg->dedi_info_nas_n_present = true;
    msg->dedi_info_nas_n.resize(n_nas);
  }
  uint32_t idx = 0;
  ...
  *zhj 0704 change
  */
  //msg->redio_resour_cfg_dedi.drb_to_add_mod_list_present=true;
    for (const drb_to_add_modi_s& drb : msg->redio_resour_cfg_dedi.drb_to_add_mod_list) {
      ...
      if (info_it != erab_info_list.end()) {
        ...
        msg->dedi_info_nas_n[idx].dedi_info_nas_n1.resize(erab_info.size());
        memcpy(msg->dedi_info_nas_n[idx].dedi_info_nas_n1.data(), &erab_info[0], erab_info.size());

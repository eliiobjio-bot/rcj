...
namespace srsenb
{
  ...
  mac::mac(srsran::ext_task_sched_handle task_sched_, srslog::basic_logger &logger) : logger(logger), rar_payload(), common_buffers(SRSRAN_MAX_CARRIERS), task_sched(task_sched_)
  {
    pthread_rwlock_init(&rwlock, nullptr);
    // initResourceMap();
    stack_task_queue = task_sched.make_task_queue();
    // area_mode=area_;
    // std::cout<<"ssssss"<<area_mode<<std::endl;
  }
                 ...
                 rrc_interface_mac *rrc, adp *mac_adp_)
  {
    ...
    ttcn_mac_enble = args.ttcn_mac_enble;
    phy_h->trans_pid_to_phy(args_.pid);
    if (ttcn_mac_enble)
    {
      printf("ttcn_mac_enble is true\n");
    }
    lastMuiltSource.reserve(16);//hm
    network_mode = args.network_mode;
    area_mode = args.area_mode;
    multi_beam_num = args.multi_beam_num;
    scheduler.init(rrc, args.sched);
    sychInit();
    std::cout << "------initsched----------" << std::endl;
    wxScheduler.init(this, area_mode, args.network_mode, multi_beam_num);
    ...
    for (auto &cc : common_buffers)
    {
      ...
      std::cout << "ssssssinit" << area_mode << std::endl;
    }
    ...
    auto recycle_softbuffers = [](ue_cc_softbuffers &softbuffers)
    { softbuffers.clear(); };
  ...
  void mac::start_pcap_net(srsran::pcap_net *pcap_net_)
  {
    srsran::rwlock_read_guard lock(rwlock);
    ppcap_net = pcap_net_;
    // Set pcap in all UEs for UL messages
    for (auto &u : ue_db)
    {
      u.second->start_pcap_net(ppcap_net);
    }
  }
  ...
  {
    std::cout << "1111111111111wcb" << std::endl;
    ...
    std::cout << "2222222222222wcb" << std::endl;
    // ue_db.erase(rnti);//ZHJ ZHJ
    scheduler.ue_rem(rnti);
    std::cout << "2222222222222wcb" << std::endl;
    // phy_h->rem_rnti(rnti);
    std::cout << "[ue_category != 14][deallocate_source]" << std::endl;
    if (ue_category == 14)
    {
      wxScheduler.UE_14_Deallocate_Source(ue_cap14_band_id, ue_cap14_freq_id, ue_cap14_slot_phy);
    }
    else
    {
      wxScheduler.deallocate_source(dlAlloc);
    }
    std::cout<<"lastMuiltSource size=:"<<lastMuiltSource.size()<<std::endl;
    ...
    //***************************************************
    // 620
    judge_rach_detect_counter = false;
    crc_check_failure_counter=0;
    // wxScheduler.wx_sched_reconfig(false,false); //realse
    ue_category = 0;
    mac_rnti = 0;
    wxScheduler.reset_uecap_after_release(0);
    //****************************************************
    ...
    // //task_sched.defer_callback(0, [this, rnti]() {
    //   phy_h->rem_rnti(rnti);
    //   srsran::rwlock_write_guard lock(rwlock);
    //   ue_db.erase(rnti);
  ...
  uint16_t mac::allocate_ue(uint32_t enb_cc_idx)
  {
    ...
    do
    {
      ...
      rnti = FIRST_RNTI;
      std::cout << "allocate_ue test 1" << std::endl;
      // Pre-check if rnti is valid
      {
        std::cout << "allocate_ue test 2" << std::endl;
        srsran::rwlock_read_guard read_lock(rwlock);
        std::cout << "allocate_ue test 3" << std::endl;
        // zhj print
        if (not is_valid_rnti_unprotected(rnti))
        {
          std::cout << "allocate_ue test 4" << std::endl;
          continue;
        }
      }
      std::cout << "allocate_ue test 5" << std::endl;
      // Allocate and initialize UE object
      unique_rnti_ptr<ue> ue_ptr = make_rnti_obj<ue>(
          rnti, rnti, 0, &scheduler, rrc_h, rlc_h, phy_h, logger, cells.size(), softbuffer_pool.get());
      // Add UE to rnti map
      std::cout << "allocate_ue test 6" << std::endl;
      if (not is_valid_rnti_unprotected(rnti))
      {
        continue;
      }
      auto ret = ue_db.insert(rnti, std::move(ue_ptr));
      std::cout << "allocate_ue test 8" << std::endl;
      if (ret.has_value())
      {
        std::cout << "allocate_ue test 9" << std::endl;
    ...
    std::cout << "allocate_ue test 10" << std::endl;
    ...
    {
      inserted_ue->start_pcap_net(ppcap_net);
    }
    // std::cout << "allocate_ue test 11" << std::endl;
    return rnti;
  }
  ...
  //      continue;
  //     }
  //     std::cout<<"allocate_ue test 7"<<std::endl;
  ...
  int mac::get_dl_sched(uint32_t tti_tx_dl, dl_sched_list_t &dl_sched_res_list)
  {
    ...
    for (uint32_t enb_cc_idx = 0; enb_cc_idx < cell_config.size(); enb_cc_idx++)
    {
      ...
      for (uint32_t i = 0; i < sched_result.rar.size(); i++)
      {
        ...
        if (ppcap_net)
        {
          ppcap_net->write_dl_ranti(dl_sched_res->pdsch[n].data[0],
                                    sched_result.rar[i].tbs,
                                    dl_sched_res->pdsch[n].dci.rnti,
                                    true,
                                    tti_tx_dl,
      ...
      for (uint32_t i = 0; i < sched_result.bc.size(); i++)
      {
        ...
        else
        {
          ...
          if (ppcap_net)
          {
            ppcap_net->write_dl_pch(dl_sched_res->pdsch[n].data[0], sched_result.bc[i].tbs, true, tti_tx_dl, enb_cc_idx);
          }
        }

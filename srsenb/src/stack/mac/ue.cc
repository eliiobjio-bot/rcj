...
namespace srsenb
{
  ...
  ue_cc_softbuffers::ue_cc_softbuffers(uint32_t nof_prb, uint32_t nof_tx_harq_proc_, uint32_t nof_rx_harq_proc_) : nof_tx_harq_proc(nof_tx_harq_proc_), nof_rx_harq_proc(nof_rx_harq_proc_)
  {
  ...
  ue::ue(uint16_t rnti_,
         ...
         uint32_t nof_cells_,
         srsran::obj_pool_itf<ue_cc_softbuffers> *softbuffer_pool_) : rnti(rnti_),
                                                                      sched(sched_),
                                                                      rrc(rrc_),
                                                                      rlc(rlc_),
                                                                      phy(phy_),
  ...
  // add sych
  uint32_t ue::get_ci_check_state()
  {
    std::cout << "ue.cc CI" << CI << std::endl;
    return CI;
  }

  void ue::start_pcap_net(srsran::pcap_net *pcap_net_)
  {
    ppcap_net = pcap_net_;
  }
  ...
  void ue::parse_packet(srsran::unique_byte_buffer_t pdu)
  {
    //  uint32_t lcid=*(pdu->msg)&0x3f;
  }
  uint32_t ue::generate_mcs_ce(uint32_t &lcid, uint8_t *payload)
  {
    if (!mcs_adjust.is_ready)
      return 0;
    lcid = 2;
    uint8_t *init = payload;
    *payload = mcs_adjust.mcs << 5 | mcs_adjust.fn_mcs << 2;
    printf("mac_mcs:%d  %d\n", mcs_adjust.mcs, *payload);
    std::cout << "mac_mcs:" << (int)mcs_adjust.mcs << "  " << (int)*payload << std::endl;
    payload++;
    mcs_adjust.is_ready = false;
    return payload - init;
  }
...
// Unpack ULSCH MAC PDU
#if 1
    // std::cout<<"start process_pdu"<<std::endl;
    //  for(int i=0;i<(int)pdu->size();i++)//测试上传的PDU是否正确
    //   {
    //    printf("bitset<8>(*(pdu->msg+i)):%04X\n",(*(pdu->msg+i)));
    //    //std::cout<<std::bitset<8>(*(pdu->msg+i))<<std::endl;yij
    //   }
#endif
    mac_msg_ul.init_rx(pdu->size(), true);
    mac_msg_ul.parse_packet(pdu->data());

    // std::cout<<"mac_msg_ul.get()->get_payload_size() = "<<mac_msg_ul.get()->get_payload_size()<<std::endl;
    ...
    if (ppcap_net != nullptr)
    {
      ppcap_net->write_ul_crnti(pdu->data(), pdu->size(), rnti, true, last_tti, ue_cc_idx);
    }
    ...
    while (mac_msg_ul.next())
    {
      ...
      std::cout << " S1 payload size " << mac_msg_ul.get()->get_payload_size() << std::endl;
      if (mac_msg_ul.get()->is_sdu())
      {
        ...
        if (route_pdu)
        {
          std::cout << "rlc  lcid@@@@@@@@@@@" << mac_msg_ul.get()->get_sdu_lcid() << std::endl;
          std::cout << "mac_msg_ul.get()->get_payload_size() = " << mac_msg_ul.get()->get_payload_size() << std::endl;
          rlc->write_pdu(rnti,
                         mac_msg_ul.get()->get_sdu_lcid(),
                         mac_msg_ul.get()->get_sdu_ptr(),
                         mac_msg_ul.get()->get_payload_size());
          std::cout << "mac_msg_ul.get()->get_payload_size() xx= " << mac_msg_ul.get()->get_payload_size() << std::endl;
        }
        //   cesi####################################################
        // std::cout<<"LCID:"<<(int)mac_msg_ul.get()->get_sdu_lcid()<<std::endl;
        // for(int  i=0;i<(int)mac_msg_ul.get()->get_payload_size();i++)
        // {
        //   std::cout<<std::bitset<8>(*(mac_msg_ul.get()->get_sdu_ptr()+i))<<std::endl;
        // }
    ...
    while (mac_msg_ul.next())
    {
      ...
      if (!mac_msg_ul.get()->is_sdu())
      {
        if (mac_msg_ul.get()->get_sdu_lcid() == 8)
        {
          // mac_msg_ul.get()->sychCI =  &0xc0;
          // std::cout<<"get sychCI: "<<mac_msg_ul.get()->sychCI<<"CI: "<<CI<<std::endl;
          CI = mac_msg_ul.get()->sychCI;
  ...
  bool ue::process_ce(srsran::sch_subh *subh, uint32_t grant_nof_prbs)
  {
    ...
    case srsran::ul_sch_lcid::LONG_BSR:
      ...
      break;
    //2024/8/16------------wwh-----------
    case srsran::ul_sch_lcid::LRNTI:
      printf("4444444444444jjjjjcc444444\n");
      handover_para->is_handover_falg = true;
      printf("is handover msg = %d\n",handover_para->is_handover_falg);
      handover_para->lrnti_bandid=subh->get_lrnti_bandid();
      handover_para->lrnti_freqid=subh->get_lrnti_freqid();
      handover_para->lrnti_srnti=subh->get_lrnti_srnti();
      printf("lrnti_bandid=%d lrnti_freqid=%d lrnti_srnti=%d\n",
      handover_para->lrnti_bandid,handover_para->lrnti_freqid,handover_para->lrnti_srnti);
      break;
      //------------------------------------
    default:
  ...
  handover_para_t* ue::get_handover_para(){
    printf("进入get_handover_para函数\n");
    return handover_para;
  }
  ...
  void ue::allocate_sdu(srsran::sch_pdu *pdu, uint32_t lcid, uint32_t total_sdu_len)
  {
    ...
    if (sdu_space > 0)
    {
      ...
      int n = 1;
      while (sdu_len >= min_sdu_len && n > 0)
      { // minimum size is a single RLC AM status PDU (2 Byte)
        if (pdu->new_subh())
        { // there is space for a new subheader
          logger.debug("SDU:   set_sdu(), lcid=%d, sdu_len=%d, sdu_space=%d", lcid, sdu_len, sdu_space);
          n = pdu->get()->set_sdu(lcid, sdu_len, this);
          if (n > 0)
          { // new SDU could be added
            sdu_len -= n;

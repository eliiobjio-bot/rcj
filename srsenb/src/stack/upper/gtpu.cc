...
int gtpu::init(const gtpu_args_t& gtpu_args, pdcp_interface_gtpu* pdcp_)
{
  ...
  auto rx_callback = [this](srsran::unique_byte_buffer_t pdu, const sockaddr_in& from) {
    xw_handle_gtpu_s1u_rx_packet(std::move(pdu), from);
  };
...
void gtpu::xw_handle_gtpu_s1u_rx_packet(srsran::unique_byte_buffer_t pdu, const sockaddr_in& addr)
{
  gtpu_count++;
  printf("[gtpu]: xw_handle_gtpu_s1u_rx_packet %d\n", gtpu_count);
  printf("[gtpu]: ip_data size: %d\n", (int)pdu->N_bytes);
  int n = (int)pdu->N_bytes > 20 ? 20 : (int)pdu->N_bytes;
  for(int i = 0; i < n; i++) 
  {
    printf("%x ", pdu->msg[i]);
  }
  printf("\n");
  pdcp->write_sdu(70, 3, std::move(pdu), -1);

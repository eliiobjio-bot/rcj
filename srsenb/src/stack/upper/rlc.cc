...
namespace srsenb
{
  ...
  void rlc::mod_ttcn_lcid(uint32_t lcid, size_t lcid_size)
  {
    ttcn_lcid_cfg.ttcn_lcid_flag = true;
    ttcn_lcid_cfg.ttcn_lcid = lcid;
    ttcn_lcid_cfg.ttcn_size = lcid_size;
  }

  void rlc::reset_ttcn_lcid()
  {
    ttcn_lcid_cfg.ttcn_lcid_flag = false;
    ttcn_lcid_cfg.ttcn_lcid = 0;
    ttcn_lcid_cfg.ttcn_size = 0;
  }
  ...
  void rlc::psch_test(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t psch_pdu)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(rnti))
    {
      if (rnti != SRSRAN_MRNTI)
      {
        users[rnti].rlc->write_sdu(lcid, std::move(psch_pdu));
      }
      else
      {
        users[rnti].rlc->write_sdu_mch(lcid, std::move(psch_pdu));
      }
    }
    pthread_rwlock_unlock(&rwlock);
  }
  ...
  /// ttcn
  void rlc::pdcp_ttcnmsg_rlc_s(uint16_t ttcn_rnti, uint32_t lcid, srsran::unique_byte_buffer_t ttcn_rlc_msg)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(ttcn_rnti))
    {
      if (ttcn_rnti != SRSRAN_MRNTI)
      {
        uint16_t rlc_testID = ((uint16_t)ttcn_rlc_msg->msg[3] << 8) | ((uint16_t)ttcn_rlc_msg->msg[4]);
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->mod_enble(true);
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->test_ID_enble(rlc_testID);
        // remove header
        ttcn_rlc_msg->N_bytes -= 21;
        ttcn_rlc_msg->msg += 21;

        users[ttcn_rnti].rlc->write_sdu(lcid, std::move(ttcn_rlc_msg));
        // mac->rlcStateReport(lcid, ttcn_rnti);//notify mac
      }
      else
      {
        users[ttcn_rnti].rlc->write_sdu_mch(lcid, std::move(ttcn_rlc_msg));
      }
    }
    pthread_rwlock_unlock(&rwlock);
  }

  void rlc::TC_6115_RLC_Handle(uint16_t ttcn_rnti, uint32_t lcid)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(ttcn_rnti))
    {
      if (ttcn_rnti != SRSRAN_MRNTI)
      {
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->TC_6115_RLC_Handle_t();
      }
      else
      {
        std::cout << " config parameters failure ! " << std::endl;
      }
    }
    pthread_rwlock_unlock(&rwlock);
  }
  void rlc::TC_6116_RLC_Handle(uint16_t ttcn_rnti, uint32_t lcid)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(ttcn_rnti))
    {
      if (ttcn_rnti != SRSRAN_MRNTI)
      {
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->TC_6116_RLC_Handle_t();
      }
      else
      {
        std::cout << " config parameters failure ! " << std::endl;
  ...
  void rlc::TC_6117_RLC_Handle(uint16_t ttcn_rnti, uint32_t lcid)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(ttcn_rnti))
    {
      if (ttcn_rnti != SRSRAN_MRNTI)
      {
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->TC_6117_RLC_Handle_t();
      }
      else
      {
        std::cout << " config parameters failure ! " << std::endl;
  ...
  void rlc::enable_rlc_testcase_related_configuration(uint16_t ttcn_rnti, uint32_t lcid, uint16_t rlc_testID,int pid)
  {
    pthread_rwlock_rdlock(&rwlock);
    if (users.count(ttcn_rnti))
    {
      if (ttcn_rnti != SRSRAN_MRNTI)
      {
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->mod_enble(true);
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->test_ID_enble(rlc_testID);
        std::cout<<" setRlcPid pid="<<pid<<std::endl;
        users[ttcn_rnti].rlc->rlc_array.at(lcid)->setRlcPid(pid);
      }
      else
      {
        std::cout << " config parameters failure ! " << std::endl;

...
namespace srsenb
{
  void init_enb_args(all_args_t &args, const all_args_t &args_)
  {

    // args.stack = args_.stack;
    args.stack.mac_pcap_net.enable = args_.stack.mac_pcap_net.enable;
    args.stack.mac_pcap_net.client_ip = args_.stack.mac_pcap_net.client_ip;
    args.stack.mac_pcap_net.bind_ip = args_.stack.mac_pcap_net.bind_ip;

    args.stack.mac_pcap_net.client_port = args_.stack.mac_pcap_net.client_port;
    args.stack.mac_pcap_net.bind_port = args_.stack.mac_pcap_net.bind_port;

    return;
  }

  enb::enb(srslog::sink &log_sink) : started(false), log_sink(log_sink), enb_log(srslog::fetch_basic_logger("ENB", log_sink, false)), sys_proc(enb_log)
  {
    // print build info
    std::cout << std::endl
              << get_build_string() << std::endl
              << std::endl;
  }
  ...
  int enb::init(const all_args_t &args_, adp *adp_)
  {
    int ret = SRSRAN_SUCCESS;
    printf("T1 args pid = %d\n", args_.pid);
    init_enb_args(args, args_);
    if (started)
    {
      srsran::console("ENB is already running.\n");
      return SRSASN_SUCCESS;
    }
    enb_adp = adp_;
    // Init eNB log
    set_logger_enable(args_.log.log_enabled);
    ...
    if (tmp_eutra_stack)
    {
      if (tmp_eutra_stack->init(args.stack, rrc_cfg, tmp_phy.get(), x2.get(),
                                enb_adp) != SRSRAN_SUCCESS)
      {
    ...
    {
      // debug20240124
      phy_cfg.band = rrc_cfg.sib.rr_cfg_com.rach_cfg_com.band_id.ba_id.to_number();
      phy_cfg.cc = rrc_cfg.sib.rr_cfg_com.rach_cfg_com.freq_bit_map.to_number();
      phy_cfg.slot = rrc_cfg.sib.rr_cfg_com.rach_cfg_com.rach_slot_ass.to_number();
      std::cout << "phy_cfg.band=" << phy_cfg.band << std::endl;
      std::cout << "phy_cfg.cc=" << phy_cfg.cc << std::endl;
      std::cout << "phy_cfg.slot=" << phy_cfg.slot << std::endl;

      std::cout << "args_.pid:" << args_.pid << std::endl;
      phy_cfg.pid = args_.pid;
      std::cout << "phy_cfg.pid:" << phy_cfg.pid << std::endl;

      phy_cfg.cc1 = rrc_cfg.sib.rr_cfg_com.rach_cfg_com.freqBitmap_FrameFull.to_number();
  ...
  void enb::set_pcap_remote_addr(std::string &ip, uint16_t port)
  {
    (dynamic_cast<enb_stack_lte *>(eutra_stack.get()))->set_pcap_remote_addr(ip, port);
    return;
  }
  bool enb::set_logger_enable(bool enable)
  {
    // srslog::fetch_basic_logger("MAC", log_sink)
    //(dynamic_cast<enb_stack_lte*>(eutra_stack.get()))->set_logger_enable(enable);
    srslog::set_global_logger_status(enable);
    if (eutra_stack != nullptr)
    {
      enb_stack_lte *ptmp = dynamic_cast<enb_stack_lte *>(eutra_stack.get());
      ptmp->test_pcap_pkt();
      srsran::console("send pcap pkt!");
    }

    return true;
  }

  int enb::get_beam_state()
  {
    if (started)
    {
      return 1;

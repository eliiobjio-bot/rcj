...
#ifdef BUILD_WITH_CY_SAT
#include "../hdr/cy_scpi_channel.h"
#include "../hdr/cy_scpi_server.h"
...
void parse_args(all_args_t *args, int argc, char *argv[])
{
  ...
  common.add_options()
    ...
    ("enb.get_rlc_mode",                bpo::value<uint32_t>(&args->enb.get_rlc_mode)->default_value(255),                  "select rlc_mode")
    ("enb.multi_beam_num",                bpo::value<uint32_t>(&args->enb.multi_beam_num)->default_value(1),                  "select beamnum")
    ("enb.network_mode",                bpo::value<uint32_t>(&args->enb.network_mode)->default_value(0),                  "select network")
    ("enb.area_mode",                bpo::value<uint32_t>(&args->enb.area_mode)->default_value(0),                  "select area")
    ("enb.ttcn_time",                bpo::value<uint32_t>(&args->enb.ttcn_time)->default_value(0),                  "set ttcn test time")

    //-------------------0929------------------
    ("enb.pid",                bpo::value<uint16_t>(&args->enb.pid)->default_value(0),                  "pid")
    ("enb.cnw_clientaddr_port",                bpo::value<uint16_t>(&args->enb.cnw_clientaddr_port)->default_value(0),                  "cnw_clientaddr_port")
    ("enb.ttcn_clientaddr_port",                bpo::value<uint16_t>(&args->enb.ttcn_clientaddr_port)->default_value(0),                  "ttcn_clientaddr_port")
    ("enb.beam1_adp_port",                bpo::value<uint16_t>(&args->enb.beam1_adp_port)->default_value(0),                  "beam1_adp_port")
    ("enb.beam2_adp_port",                bpo::value<uint16_t>(&args->enb.beam2_adp_port)->default_value(0),                  "beam2_adp_port")
    ("enb.ate_clientaddr_port",                bpo::value<uint16_t>(&args->enb.ate_clientaddr_port)->default_value(0),                  "ate_clientaddr_port")
    ("enb.control_clientaddr_port",                bpo::value<uint16_t>(&args->enb.control_clientaddr_port)->default_value(0),                  "control_clientaddr_port")
    ("enb.cnw_clientaddr_ip",             bpo::value<string>(&args->enb.cnw_clientaddr_ip)->default_value("127.0.0.1"),                       "cnw_clientaddr_ip")
    ("enb.ttcn_clientaddr_ip",             bpo::value<string>(&args->enb.ttcn_clientaddr_ip)->default_value("127.0.0.1"),                       "ttcn_clientaddr_ip")
    ("enb.target_adp_ip",             bpo::value<string>(&args->enb.target_adp_ip)->default_value("127.0.0.1"),                       "cnw_clientaddr_ip")
    ("enb.ate_clientaddr_ip",             bpo::value<string>(&args->enb.ate_clientaddr_ip)->default_value("127.0.0.1"),                       "ate_clientaddr_ip")
    ("enb.control_clientaddr_ip",             bpo::value<string>(&args->enb.control_clientaddr_ip)->default_value("127.0.0.1"),                       "control_clientaddr_ip")
//---------------------------------------------

    ("enb.scpi_tcp_port", bpo::value<uint32_t>(&args->enb.scpi_tcp_port)->default_value(33770), "The scpi server tcp port")
...
//-----------------2022/07/29---------------
    ("enb_files.sib_wx_config", bpo::value<string>(&args->enb_files.sib_wx_config)->default_value("sib_wx.conf"), "SIB configuration files")
    ("enb_files.mib_config", bpo::value<string>(&args->enb_files.mib_config)->default_value("mib.conf"), "MIB configuration files")
//------------------------------------------

 //--------------2024.06.22----------------
        ("enb_files.recfg_voice_config", bpo::value<string>(&args->enb_files.recfg_voice_config)->default_value("recfg_voice.conf"), "voice_recfg configuration files")
      //---------------------------

 //--------------2024.06.22----------------
        ("enb_files.recfg_kuopin_config", bpo::value<string>(&args->enb_files.recfg_kuopin_config)->default_value("recfg_voice_kuopin.conf"), "kuopin_recfg configuration files")
        ("enb_files.recfg_norm_kuopin_config", bpo::value<string>(&args->enb_files.recfg_norm_kuopin_config)->default_value("recfg_norm_kuopin.conf"), "normal kuopin recfg configuration files")
      //---------------------------


 //--------------2024.03.22----------------
        ("enb_files.recfg_wx_config", bpo::value<string>(&args->enb_files.recfg_wx_config)->default_value("recfg_wx.conf"), "recfg configuration files")
      //---------------------------
    ...
    ("rf.ksw_channel_choose",   bpo::value<uint32_t>(&args->rf.ksw_channels)->default_value(0), "ksw channel choose, 0: chanel1, 1: channel2")
  ...
  #if 1 //BUILD_WITH_CY_SAT
    ("log.remote_ip",      bpo::value<string>(&args->log.remote_ip)->default_value("127.0.0.1"),"Log remote udp ip")
    ("log.remote_port", bpo::value<int>(&args->log.remote_port)->default_value(6666), "Log remote udp port")
    ("log.log_enabled", bpo::value<bool>(&args->log.log_enabled)->default_value(true), "Enable the logger")
    ...
    ("expert.nof_phy_threads", bpo::value<uint32_t>(&args->phy.nof_phy_threads)->default_value(2), "Number of PHY threads.")
    ...
    ("expert.rrc_inactivity_timer", bpo::value<uint32_t>(&args->general.rrc_inactivity_timer)->default_value(25000000), "Inactivity timer in ms.")
    ...
    ("expert.eia_pref_list", bpo::value<string>(&args->general.eia_pref_list)->default_value("EIA0, EIA1, EIA2"), "Ordered preference list for the selection of integrity algorithm (EIA) (default: EIA0, EIA1, EIA2).")
  ...
  //--------------------------2022/07/29-------------------------
  if (!config_exists(args->enb_files.sib_wx_config, "sib_wx.conf"))
  {
    cout << "Failed to read SIB configuration file " << args->enb_files.sib_wx_config << " - exiting" << endl;
    exit(1);
  }
  if (!config_exists(args->enb_files.mib_config, "mib.conf"))
  {
    cout << "Failed to read MIB configuration file " << args->enb_files.mib_config << " - exiting" << endl;
    exit(1);
  }
  //-------------------------------------------------------------

  //---------------2024.06.22-----------------
  if (!config_exists(args->enb_files.recfg_voice_config, "recfg_voice.conf"))
  {
    std::cout << "fhjdfhjdkshfdhjkvoicecee" << std::endl;
    cout << "Failed to read recfg voice configuration file " << args->enb_files.recfg_voice_config << " - exiting" << endl;
    exit(1);
  }
  std::cout << "args->enb_files.recfg_voice_config" << args->enb_files.recfg_voice_config << std::endl;

  //---------------2024.06.22-----------------
  if (!config_exists(args->enb_files.recfg_kuopin_config, "recfg_voice_kuopin.conf"))
  {
    std::cout << "fhjdfhjdkshfdhjkvoiceceekuopinjfhj" << std::endl;
    cout << "Failed to read recfg voice configuration file " << args->enb_files.recfg_kuopin_config << " - exiting" << endl;
    exit(1);
  }
  std::cout << "args->enb_files.recfg_kuopin_config;" << args->enb_files.recfg_kuopin_config << std::endl;

  //---------------2024.03.22-----------------
  if (!config_exists(args->enb_files.recfg_wx_config, "recfg_wx.conf"))
  {
    std::cout << "fhjdfhjdkshfdhjk" << std::endl;
    cout << "Failed to read recfg configuration file " << args->enb_files.recfg_wx_config << " - exiting" << endl;
    exit(1);
  }
  //----------------------------------------------------

  if (!config_exists(args->enb_files.recfg_norm_kuopin_config, "recfg_norm_kuopin.conf"))
  {
    std::cout << "fhjdfhjdkshfdhjkvoiceceenorm_kuopin" << std::endl;
    cout << "Failed to read recfg normal kuopin configuration file " << args->enb_files.recfg_norm_kuopin_config << " - exiting" << endl;
    exit(1);
  }
  std::cout << "args->enb_files.recfg_norm_kuopin_config" << args->enb_files.recfg_norm_kuopin_config << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  /**
   *@brief  change ADP location
   *@author zhaohongjun
   *@date   2024/06/03
   */
  /*
    auto start_time = std::chrono::high_resolution_clock::now();
    std::cout << "-----Init adp moule-----" << std::endl;
    adp_.init(&args);
  */
  ...
  cout << "---  Software Radio Systems LTE eNodeB  ---" << endl
       << endl;

  srsran_debug_handle_crash(argc, argv);
  parse_args(&args, argc, argv);
  std::cout<<" A config_file = "<<config_file<<std::endl;
  args.config_file=config_file;
  std::cout<<" B args.config_file = "<<args.config_file<<std::endl;

  //-------------------0929------------------
  args.pid = args.enb.pid;
  printf("args pid = %d\n", args.pid);
  printf("pid=%d\n", args.enb.pid);
  ...
  // 2024/06/03 after update location
  auto start_time = std::chrono::high_resolution_clock::now();
  std::cout << "-----Init adp moule-----" << std::endl;
  adp_.init(&args);
 printf("T2 args pid = %d\n", args.pid);
  // Setup the default log sink.
#if 0 // BUILD_WITH_CY_SAT
  srslog::set_default_sink(
      *srslog::create_udp_sink("remote_udp", args.log.remote_ip, static_cast<uint16_t>(args.log.remote_port)));
#else
  srslog::set_default_sink(
  ...
  while (true)
  {
    ...
    if (args.ttcn_test_enble)
    { // Upon receipt of the first message, the
      // startup flag is activated enb
      if (enb->init(args, &adp_) != SRSRAN_SUCCESS)
      {
        enb->stop();
        return SRSRAN_ERROR;
      }
    ...
    // if (duration.count() > MAX_TIME) {  // If no message is received within a certain
    if (duration.count() > args.enb.ttcn_time)
    { // If no message is received within a certain
      // period, the system starts directly
      args.not_ttcn_test = true;
      if (enb->init(args, &adp_) != SRSRAN_SUCCESS)
      {
        enb->stop();
        return SRSRAN_ERROR;
...
#ifdef BUILD_WITH_CY_SAT
  std::shared_ptr<SCPI_SERVER> pScpiServer = std::make_shared<SCPI_SERVER>(srslog::get_default_sink(), (uint16_t)args.enb.scpi_tcp_port);
  pScpiServer->init();
  if (CYSCPI_CHANNEL::mp_scpi == nullptr)
  {
    CYSCPI_CHANNEL::mp_scpi = std::make_shared<CYSATScpiParse>(enb.get(), &args);
  }
...
// create input thread
  ...
  enb->stop();
#ifdef BUILD_WITH_CY_SAT
  cout << "---  exiting scpi server ---" << endl;
  pScpiServer->stop();
  cout << "---  exited scpi server ---" << endl;
#endif
  cout << "---  exiting  ---" << endl;

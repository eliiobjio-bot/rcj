...
public:
  ...
  // V1.0.0#################################
int  get_sched(uint32_t tti,  std::vector<sched_t>&dl_sched_res, uint32_t prach_tti) {return 0;}
int  get_IoT_sched(uint32_t tti,  std::vector<sched_t>&dl_sched_res) {return 0;}
int iot_rach_detected(iot_prachInfo_t* iot_prachInfo, uint8_t* pdu, int pduLen){return 0;}
int rach_detected(prachInfo_t* prachInfo, uint8_t* pdu, int pduLen){return 0;}
int sych_detected(psychInfo_t* psychInfo){return 0;}
int push_pdu(uint32_t tti_rx,uint16_t rnti,uint8_t *msg,uint32_t pduLen,bool  crc,uint8_t voice) {return 0;};
//#################################################
  ...
  int push_pdu(uint32_t tti, uint16_t rnti, uint32_t cc_idx, uint32_t nof_bytes, bool crc_res, uint32_t grant_nof_prbs,uint8_t voice)
      override
  {

...
namespace srsenb
{
  ...
  public:
    ...
    bool setConsID(set_up_msg setUp, dl_allocate allocSource, ul_allocate ulAlloc) { return false; };
    bool TC300Timeout(set_up_msg setUp) { return false; }
    bool setupInfo_uecategory14(set_up_msg setUp) { return false; }
    void addlcidMap(uint32_t lcid, configMap configMap_) { std::cout << "This addlcidMap mac_dummy" << std::endl; }
    // void updateMap(uint32_t lcid,ChanType_t reconfig_new_ul_Type,ChanType_t reconfig_new_dl_Type){std::cout<<"This updateMap mac_dummy"<<std::endl;}
    void updateMap(uint32_t lcid, configMap updateMap_) { std::cout << "This updateMap mac_dummy" << std::endl; };
    bool getVoiceLcid(uint32_t lcid, int voice_speed) { return false; }
    bool setUecategory(uint8_t uecategory, uint16_t ue_cap14_band_id_, uint16_t ue_cap14_freq_id_, uint16_t ue_cap14_slot_, uint16_t ue_cap14_slot_no_handle_) { return false; }
    bool getSrnti(int srnti) { return false; }
    //--------------------2023.12.15------------------------------------
    bool reconf_phy(dl_allocate allocSource, ul_allocate alloc, mcs_Info mcs_Info_, int Handover_frame_off = 0) { return false; };
    //-------------------2024.09.04-----------
    bool phy_channel_list_conf(phy_channel_list_t phy_channel_list, phy_channel_ul_list_t phy_ul_channel_list, mcs_Info mcs_Info_) { return false; };

    //---------------------------------------------------
    // 2024.11.12 to notify PHY resolution in advance
 

    void mac_deallocate() { return; };
    bool wx_Switch_SetUser_in_TargetBeam()
    {
      std::cout << "dummy class wx_Switch_SetUser_in_TargetBeam" << std::endl;
      return true;
    }
    //---------------------------------------------------

    //---------------------------------------------------
    void RRC_notify_MAC_release(bool is_mac_chanType_release_) { std::cout << " NULL " << std::endl; }

    void rlcStateReport(int lcid, int rnti) { return; };
  ...
  public:
    ...
    //----------------2023.10.30--------------------
    void s_set_config(uint16_t rnti, const phy_rrc_cfg_list_t &dedicated_list) override {}
    //--------------------------------------------------------------
